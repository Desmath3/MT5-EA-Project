...
input bool   UseMACDFilter       = true;        // Use MACD filter for trade confirmation
...
public:
   ...
   ~CBBReversalEA()
     {
      if(m_BandHandle != INVALID_HANDLE) IndicatorRelease(m_BandHandle);
   ...
   bool InitializeIndicators()
     {
      ...
      m_MACDHandle = iMACD(_Symbol, _Period, m_MACDFastPeriod, m_MACDSlowPeriod, m_MACDSignalPeriod, PRICE_CLOSE);
      if(m_BandHandle == INVALID_HANDLE || m_ATRHandle == INVALID_HANDLE || m_MACDHandle == INVALID_HANDLE)
        {
   ...
   bool GetMACDValues(int bar, double macd, double signal)
     {
      double macdArr[1], signalArr[1];
      if(CopyBuffer(m_MACDHandle, 0, bar, 1, macdArr) != 1) return false;
      if(CopyBuffer(m_MACDHandle, 1, bar, 1, signalArr) != 1) return false;
      macd = macdArr[0];
      signal = signalArr[0];
      return true;
     }
   ...
   void ManageTrades()
     {
      ...
      if(open1 > upperArr[0] && close1 < upperArr[0]) sig = ORDER_TYPE_SELL;
      else if(open1 < lowerArr[0] && close1 > lowerArr[0]) sig = ORDER_TYPE_BUY;
      if(sig == (ENUM_ORDER_TYPE)-1) return;

      if(m_UseMACDFilter)
        {
         double macd, signal;
         if(!GetMACDValues(1, macd, signal)) return;
         if((sig == ORDER_TYPE_BUY && macd < 0) || (sig == ORDER_TYPE_SELL && macd > 0)) return;
        }
      ...
      if(!HasOpenOfType(ORDER_TYPE_BUY) && !HasOpenOfType(ORDER_TYPE_SELL) )
        {
         OpenTrade(sig);
         m_tradesToday++;
        }
