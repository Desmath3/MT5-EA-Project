//+------------------------------------------------------------------+
//|                          Limitless EA BB V4.36.mq5               |
//|                                  Copyright 2025, Ighodalo        |
//|                                        @--ighodalo on X         |
//+------------------------------------------------------------------+
#property copyright "Ighodalo"
#property link      "https://www.mql5.com"
#property version   "4.36" // Added opposite BB exit alongside ATR partials; replaced long loss exit.
#property description "Trades Bollinger Bands reversals with power candle confirmation and multi-timeframe filters."
...
input bool CheckATRDistance = true;    // Check if entry is too far from BB
input double MaxATRDistanceMultiplier = 1.4; // Max distance from BB = ATR * Multiplier

// --- Risk & Position Management ---
input group "Risk & Position Management"
input ENUM_ORDER_EXECUTION_TYPE OrderExecutionType = EXECUTION_TYPE_MARKET; // How to enter trades
...
input double PartialCloseAtrMultiplier = 1.5; // ATR multiple profit target for partial close
...
input int PendingOrderExpiryCandles = 30; // Expire pending orders after X candles
...
private:
   ...
   ENUM_ORDER_EXECUTION_TYPE m_OrderExecutionType;
   bool              m_EnableHedge;
   ...
   double            m_PartialCloseAtrMultiplier;
   ...
   PendingOrderSLInfo m_pendingSLs[];
   ...
   int               m_PendingOrderExpiryCandles;
         ...
         for(int i = OrdersTotal() - 1; i >= 0; i--)
           {
            ...
            if(OrderSelect(ticket))
              {
               if(OrderGetInteger(ORDER_MAGIC) == m_MagicNumber && OrderGetString(ORDER_SYMBOL) == _Symbol)
                 {
   ...
   void ManagePendingOrders()
     {
      for(int i = ArraySize(m_pendingOrders) - 1; i >= 0; i--)
        {
         if(!m_order.Select(m_pendingOrders[i].ticket))
           {
            ArrayRemove(m_pendingOrders, i, 1);
            continue;
           }
         int bars_since_setup = iBarShift(_Symbol, _Period, m_pendingOrders[i].setupTime);
         if(bars_since_setup >= m_PendingOrderExpiryCandles)
           {
            if(m_trade.OrderDelete(m_pendingOrders[i].ticket))
              {
               Print("Pending order #", m_pendingOrders[i].ticket, " expired and was deleted.");
               ArrayRemove(m_pendingOrders, i, 1);
              }
           }
        }
   ...
   CBBReversalEA()
     {
      ...
      m_OrderExecutionType = OrderExecutionType;
      ...
      m_PartialCloseAtrMultiplier = PartialCloseAtrMultiplier;
      ...
      m_sleepStart = 0;
      m_sleepEndTime = 0;
      m_PendingOrderExpiryCandles = PendingOrderExpiryCandles;
   ...
   ENUM_ORDER_TYPE GetTradeSignal()
     {
      if(m_TradeWickRejections)
        ...
        return GetBBReversalSignal();
     }

   ENUM_ORDER_TYPE GetBBReversalSignal()
     {
      ...
      if(m_bbState == BB_WAIT_BUY && lastClose > m_lowerBand[0])
        {
         Print("Signal Trigger: BB Reclaim BUY. Close (", DoubleToString(lastClose, _Digits), ") reclaimed the Lower Band (", DoubleToString(m_lowerBand[0], _Digits), ").");
         signal = ORDER_TYPE_BUY;
         m_bbState = BB_NONE;
        }
      else if(m_bbState == BB_WAIT_SELL && lastClose < m_upperBand[0])
        {
         Print("Signal Trigger: BB Reclaim SELL. Close (", DoubleToString(lastClose, _Digits), ") reclaimed the Upper Band (", DoubleToString(m_upperBand[0], _Digits), ").");
   ...
   ENUM_ORDER_TYPE GetWickRejectionSignal()
     {
      ...
      if(high1 > m_upperBand[0] && close1 < m_upperBand[0])
        {
         Print("Signal Trigger: BB Wick Rejection SELL. High (", DoubleToString(high1, _Digits), ") pierced Upper Band (", DoubleToString(m_upperBand[0], _Digits), ") but Close (", DoubleToString(close1, _Digits), ") was below it.");
         signal = ORDER_TYPE_SELL;
        }
      else if(low1 < m_lowerBand[0] && close1 > m_lowerBand[0])
        {
         Print("Signal Trigger: BB Wick Rejection BUY. Low (", DoubleToString(low1, _Digits), ") pierced Lower Band (", DoubleToString(m_lowerBand[0], _Digits), ") but Close (", DoubleToString(close1, _Digits), ") was above it.");
         signal = ORDER_TYPE_BUY;
        }
   ...
   bool CheckADXTrend(ENUM_ORDER_TYPE signal)
{
    ...
    else
    {
        ...
        Print("ADX Filter: BLOCKED. ADX value (", DoubleToString(adx_main[0], 2), ") is above threshold (", m_ADXThreshold, "), indicating a strong trend unsuitable for reversals.");
        return false;
    }
}
   ...
   bool CheckATRDistanceFromBB(ENUM_ORDER_TYPE signal)
     {
      ...
      if(CopyBuffer(m_BandHandle, 1, 1, 1, m_upperBand) <= 0) return false;
      if(CopyBuffer(m_BandHandle, 2, 1, 1, m_lowerBand) <= 0) return false;
      ...
      if(signal == ORDER_TYPE_BUY)
        {
         ...
         double distance = entryPrice - m_lowerBand[0];
         if(distance > maxDist)
           {
            Print("BUY Blocked: Distance from Lower BB (", DoubleToString(distance, _Digits), ") is greater than Max Allowed (", DoubleToString(maxDist, _Digits), ").");
            return false;
           }
        }
      else
        {
         ...
         double distance = m_upperBand[0] - entryPrice;
         if(distance > maxDist)
           {
            Print("SELL Blocked: Distance from Upper BB (", DoubleToString(distance, _Digits), ") is greater than Max Allowed (", DoubleToString(maxDist, _Digits), ").");
            return false;
           }
        }
   ...
   bool IsNewTradeTooClose(ENUM_ORDER_TYPE orderType)
     {
      ...
      for(int i = OrdersTotal() - 1; i >= 0; i--)
        {
         ulong ticket = OrderGetTicket(i);
         if(OrderSelect(ticket) && OrderGetInteger(ORDER_MAGIC) == m_MagicNumber && OrderGetString(ORDER_SYMBOL) == _Symbol)
           {
            if(OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_BUY_LIMIT || OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_SELL_LIMIT)
              {
               ...
               double distance = MathAbs(currentPrice - existingOrderPrice);
               if(distance < minDistance)
                 {
                  Print("Trade Blocked: New ", (orderType == ORDER_TYPE_BUY ? "BUY" : "SELL"), " signal is too close to pending order #", ticket,
                        ". Current distance: ", DoubleToString(distance, _Digits),
                        ", Required distance: ", DoubleToString(minDistance, _Digits));
                  return true;
                 }
   ...
   void CheckPartialClose()
     {
      ...
      double trigger_distance = atr * m_PartialCloseAtrMultiplier;
   ...
   void UpdateManagedTrades()
     {
      ...
      for(int i = ArraySize(m_pendingSLs) - 1; i >= 0; i--)
        {
         if(!OrderSelect(m_pendingSLs[i].order_ticket))
           {
            ...
            for(int j = HistoryDealsTotal() - 1; j >= 0; j--)
              {
               ...
               if(HistoryDealGetInteger(deal_ticket, DEAL_ORDER) == m_pendingSLs[i].order_ticket)
                 {
            ...
            if(position_ticket > 0)
              {
               int size = ArraySize(m_managedPositions);
               ArrayResize(m_managedPositions, size + 1);
               m_managedPositions[size].position_ticket = position_ticket;
               m_managedPositions[size].dynamicSL_level = m_pendingSLs[i].dynamicSL_level;
               Print("Dynamic SL: Pending order filled. Now managing position #", position_ticket);
              }
            if(position_ticket == 0)
              {
               datetime now = TimeCurrent();
               ENUM_POSITION_TYPE pos_type = m_pendingSLs[i].order_type == ORDER_TYPE_BUY ? POSITION_TYPE_BUY : POSITION_TYPE_SELL;
               for(int j = PositionsTotal() - 1; j >= 0; j--)
                 {
                  if(m_position.SelectByIndex(j) && m_position.Symbol() == _Symbol && m_position.Magic() == m_MagicNumber && m_position.PositionType() == pos_type && m_position.Time() >= now - 300)
                    {
                     bool is_already = false;
                     for(int k = 0; k < ArraySize(m_managedPositions); k++)
                       {
                        if(m_managedPositions[k].position_ticket == m_position.Ticket())
                          {
                           is_already = true;
                           break;
                          }
                     ...
                     m_managedPositions[size].position_ticket = position_ticket;
                     m_managedPositions[size].dynamicSL_level = m_pendingSLs[i].dynamicSL_level;
                     Print("Fallback for pending: Managing position #", position_ticket);
                     break;
                    }
                 }
              }
            ArrayRemove(m_pendingSLs, i, 1);
           }
        }
         ...
         if(m_position.SelectByIndex(i) && m_position.Symbol() == _Symbol && m_position.Magic() == m_MagicNumber)
           {
            ...
            if(type == POSITION_TYPE_BUY && tick.bid >= upper[0]) hit = true;
            else if(type == POSITION_TYPE_SELL && tick.ask <= lower[0]) hit = true;
   ...
   void PlaceTrade(ENUM_ORDER_TYPE orderType)
     {
      ...
      if (!m_EnableHedge && HasOpenOppositeDirectionNotAtBE(orderType)) 
        {
         Print("   - Trade Blocked: Hedging disabled and existing opposite positions not at breakeven.");
         return;
        }
      ...
      if(m_OrderExecutionType == EXECUTION_TYPE_LIMIT)
        {
         // Cancel previous limit orders in the same direction
         long limit_type = (orderType == ORDER_TYPE_BUY) ? ORDER_TYPE_BUY_LIMIT : ORDER_TYPE_SELL_LIMIT;
         for(int i = OrdersTotal() - 1; i >= 0; i--)
           {
            ulong ticket = OrderGetTicket(i);
            if(OrderSelect(ticket))
              {
               if(OrderGetInteger(ORDER_MAGIC) == m_MagicNumber && OrderGetString(ORDER_SYMBOL) == _Symbol && OrderGetInteger(ORDER_TYPE) == limit_type)
                 {
                  if(m_trade.OrderDelete(ticket))
                    {
                     Print("Canceled previous limit order #", ticket, " before placing new one.");
                    }
                 }
              }
           }

         double signalPrice = (orderType == ORDER_TYPE_BUY) ? iLow(_Symbol, _Period, 1) : iHigh(_Symbol, _Period, 1);
         if(orderType == ORDER_TYPE_BUY)
            m_trade.BuyLimit(lotSize, signalPrice, _Symbol, slPrice, tpPrice);
         else
            m_trade.SellLimit(lotSize, signalPrice, _Symbol, slPrice, tpPrice);
        }
      else
        {
         if(orderType == ORDER_TYPE_BUY)
      ...
      if(m_trade.ResultRetcode() == TRADE_RETCODE_DONE || m_trade.ResultRetcode() == TRADE_RETCODE_PLACED)
        {
         ...
         if(m_EnableDynamicSL && dynamic_sl_level > 0)
           {
            if(m_OrderExecutionType == EXECUTION_TYPE_MARKET)
              {
               ulong position_ticket = 0;
               ...
               if(position_ticket == 0)
                 {
                  ...
                  for(int i = PositionsTotal() - 1; i >= 0; i--)
                    {
                     if(m_position.SelectByIndex(i) && m_position.Symbol() == _Symbol && m_position.Magic() == m_MagicNumber && m_position.PositionType() == pos_type && m_position.Time() >= now - 60)
                       {
                        position_ticket = m_position.Ticket();
                        Print("Fallback: Found position #", position_ticket, " for dynamic SL.");
                        break;
                       }
                    }
                 }
               if(position_ticket > 0)
                 {
                  ...
                  ArrayResize(m_managedPositions, size + 1);
                  m_managedPositions[size].position_ticket = position_ticket;
                  m_managedPositions[size].dynamicSL_level = dynamic_sl_level;
            ...
            else
              {
               ...
               if(order_ticket > 0)
                 {
                  int size = ArraySize(m_pendingSLs);
                  ArrayResize(m_pendingSLs, size + 1);
                  m_pendingSLs[size].order_ticket = order_ticket;
                  m_pendingSLs[size].dynamicSL_level = dynamic_sl_level;
                  m_pendingSLs[size].order_type = orderType;

                  // Add to pending order tracker for expiration
                  int po_size = ArraySize(m_pendingOrders);
                  ArrayResize(m_pendingOrders, po_size + 1);
                  m_pendingOrders[po_size].ticket = order_ticket;
                  m_pendingOrders[po_size].setupTime = iTime(_Symbol, _Period, 0);
                 }
              }
           }
