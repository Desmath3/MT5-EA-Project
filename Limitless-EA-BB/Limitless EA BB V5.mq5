//+------------------------------------------------------------------+
//|                          Limitless EA BB V4.35.mq5               |
//|                                  Copyright 2025, Ighodalo        |
//|                                        @--ighodalo on X         |
//+------------------------------------------------------------------+
#property copyright "Ighodalo"
#property link      "https://www.mql5.com"
#property version   "4.35" // Robustly fixed partial close [Unsupported filling mode] error.
...
#property description "Exit logic now includes Dynamic SL, ATR-based partials, and market structure shift exits."
...
input bool   EnableLongLossExit = true; // Enable exit for long losing trades at opposite BB
input int    MaxLossCandles = 100; // Max candles in loss before enabling opposite BB exit
...
private:
   ...
   bool              m_EnableLongLossExit;
   int               m_MaxLossCandles;
   ...
   CBBReversalEA()
     {
      ...
      m_EnableLongLossExit = EnableLongLossExit;
      m_MaxLossCandles = MaxLossCandles;
   ...
   bool CheckADXTrend(ENUM_ORDER_TYPE signal)
     {
      ...
      if(CopyBuffer(m_ADXHandle, 0, 1, 1, adx_main) <= 0) return false;
      if(CopyBuffer(m_ADXHandle, 1, 1, 1, adx_plus) <= 0) return false;
      if(CopyBuffer(m_ADXHandle, 2, 1, 1, adx_minus) <= 0) return false;
       
      bool isTrending = adx_main[0] > m_ADXThreshold;
      bool isBullish = adx_plus[0] > adx_minus[0];
       
      if(isTrending)
        {
         if(signal == ORDER_TYPE_BUY && isBullish) return true;
         if(signal == ORDER_TYPE_SELL && !isBullish) return true;
        }
...
//+------------------------------------------------------------------+
   ...
   void CheckLongLossExit()
     {
      if(!m_EnableLongLossExit) return;
      ...
      for(int i = PositionsTotal() - 1; i >= 0; i--)
        {
         if(m_position.SelectByIndex(i) && m_position.Symbol() == _Symbol && m_position.Magic() == m_MagicNumber)
           {
            if(m_position.Profit() >= 0) continue;
            datetime open_time = m_position.Time();
            int candles = iBarShift(_Symbol, _Period, open_time, false);
            if(candles <= m_MaxLossCandles) continue;
            if(m_position.PositionType() == POSITION_TYPE_BUY)
              {
               if(tick.bid >= upper[0])
                 {
                  Print("Long Loss Exit: Closing BUY position #", m_position.Ticket(), " at Upper BB.");
                  m_trade.PositionClose(m_position.Ticket());
                 }
              }
            else
              {
               if(tick.ask <= lower[0])
                 {
                  Print("Long Loss Exit: Closing SELL position #", m_position.Ticket(), " at Lower BB.");
                  m_trade.PositionClose(m_position.Ticket());
                 }
              }
           }
