//+------------------------------------------------------------------+
//|                 Belema BB Scalper v1.0 (Hybrid)                  |
//|------------------------------------------------------------------|
//| Description: This EA combines Bollinger Band reversal entries    |
//|              with the advanced scaling and trade management      |
//|              engine from the Belema SFP series.                  |
//|                                                                  |
//|              - Entry Logic: Bollinger Bands Reversal             |
//|              - Trade Management: SFP Scaling Engine v2.7         |
//+------------------------------------------------------------------+
#property copyright "Belema & Ighodalo"
#property link      "https://www.mql5.com"
#property version   "1.0"
#property description "Hybrid EA: Bollinger Band entries with SFP scaling & management."
...
#property tester_file "Hybrid_BB_SFP_NewsCache.csv"

// --- ENUMERATIONS ---
enum ENUM_ENTRY_STRATEGY
  {
   WICK_REJECTION, // Entry on a wick piercing the band
   BAND_RECLAIM    // Entry after price closes back inside the band
  };
...
input ENUM_ENTRY_STRATEGY EntryStrategy = WICK_REJECTION; // Entry on Wick Rejection or Band Reclaim
...
input int    EA_MagicNumber = 174356;  // Unique identifier for trades managed by this EA
...
string g_news_cache_filename = "Hybrid_BB_SFP_NewsCache.csv";
...
void UpdateBBState()
  {
   ...
   if(lastClose < lowerBand[0])
     {
      g_bbState = BB_WAIT_BUY;
     }
   else if(lastClose > upperBand[0])
     {
      g_bbState = BB_WAIT_SELL;
     }
  }

bool GetTradeSignal(bool &isBuy)
  {
   if(EntryStrategy == WICK_REJECTION)
     {
      double high1 = iHigh(_Symbol, _Period, 1);
      double low1 = iLow(_Symbol, _Period, 1);
      double close1 = iClose(_Symbol, _Period, 1);
      double upperBand[1], lowerBand[1];
      if(CopyBuffer(g_bandHandle, 1, 1, 1, upperBand) <= 0) return false;
      if(CopyBuffer(g_bandHandle, 2, 1, 1, lowerBand) <= 0) return false;

      if(high1 > upperBand[0] && close1 < upperBand[0])
        {
         isBuy = false;
         return true;
        }
      else if(low1 < lowerBand[0] && close1 > lowerBand[0])
        {
         isBuy = true;
         return true;
        }
   ...
   else // BAND_RECLAIM
     {
      ...
      if(g_bbState == BB_WAIT_BUY && lastClose > lowerBand[0])
        {
         isBuy = true;
         g_bbState = BB_NONE;
         return true;
        }
      else if(g_bbState == BB_WAIT_SELL && lastClose < upperBand[0])
        {
         isBuy = false;
         g_bbState = BB_NONE;
         return true;
...
bool PlaceInitialTrade(bool isBuy)
  {
   ...
   string comment = "BB|" + (isBuy ? "BUY" : "SELL") + "|Initial";
         ...
         if(!HasOpenBuyPositions() && !HasOpenSellPositions())
           {
            ...
            if(GetTradeSignal(is_buy))
              {
               // PlaceTrade will handle internal filters
