...
input int    PendingOrderExpiryCandles = 100; // How many bars a pending order can exist before being deleted
...
input int    RangeLookback          = 5;    // Lookback period for premium/discount range
input bool   UseFVGConfirmation     = false; // Require a Fair Value Gap for entry confirmation
input int    FindFVGLimit           = 16;   // How many bars to look for an FVG after the power candle
...
input int    DynamicSLLookback        = 4;     // Lookback from the trigger candle for Dynamic SL
input bool   EnableReversalExit       = true;  // Exit trades based on market structure shift
input int    LookbackHighLow          = 100;     // Lookback for market structure shift
input bool   EnableLossReversalExit   = true;  // Enable reversal exit only when in loss
input int    LossLookbackHighLow      = 30;     // Lookback period for loss reversal exit
...
input color  BullFvgColor             = 25600; // Color for bullish FVG boxes
input color  BearFvgColor             = 139;   // Color for bearish FVG boxes
...
// Checks if all open buy positions are at breakeven or better
...
// Checks if all open sell positions are at breakeven or better
...
bool PlaceTrade(ENUM_ORDER_TYPE type, double entryPrice, double level, bool isResistance, ENUM_TIMEFRAMES tf, datetime pivotTime, int triggerCandleShift)
  {
   ...
   if(isBuyOrder && HasOpenBuyPositions() && !AreAllBuyPositionsAtBreakeven())
     {
      Print("Trade Blocked: A Buy position is already open and not at breakeven.");
      return false;
     }
   if(isSellOrder && HasOpenSellPositions() && !AreAllSellPositionsAtBreakeven())
     {
...
// Get most recent resistance on specified TF
...
// Get most recent support on specified TF
...
// Trade expiry management: Close if trade is open longer than expiry candles and retraces to entry
void CheckTradeExpiry()
  {
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ulong ticket = PositionGetTicket(i);
      if(!PositionSelectByTicket(ticket) || PositionGetInteger(POSITION_MAGIC) != EA_MagicNumber || PositionGetString(POSITION_SYMBOL) != _Symbol) continue;
      ...
      if(profit > 0)
        {
         // In profit: close on retrace to or beyond entry
        ...
        {
         // In loss: close on recovery to or beyond entry
         if(posType == POSITION_TYPE_BUY && current_price >= open_price) close_condition = true;
         if(posType == POSITION_TYPE_SELL && current_price <= open_price) close_condition = true;
        }
...
// Trailing SL management: Trail SL once per candle close after trigger
...
void OnTick()
  {
   ...
   // Clean up the obj partial closed tickets array
