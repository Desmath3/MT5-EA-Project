//+------------------------------------------------------------------+
//| Belema SFP EA (Enhanced) - v2.7 with Exponential Scaling         |
//| Swing Failure Pattern with Scaling In                            |
//| v2.7: Added optional, single-parameter exponential lot sizing.   |
...
#property description "v2.7: Added optional, damped exponential scaling logic."
...
void ResetBuyVars()
  {
   ...
   currentMultiplier_buy = 0; // Reset new variable
  }
void ResetSellVars()
  {
   initialATR_sell = 0;
   ...
   currentMultiplier_sell = 0; // Reset new variable
  }
void AddBuyPosition()
  {
...
// --- END of validation ---
   ...
   if(EnableExponentialScaling)
     {
      double local_aggression_factor = AggressionFactor; // Use a local variable
      if(local_aggression_factor <= 1.0) local_aggression_factor = 1.01; // Modify the local variable to prevent math errors

      if(positionsCount_buy == 1) // This is for adding the 2nd position
        {
         newLot = initialLot_buy * Add_Lot_Multiplier;
         // Prepare the multiplier for the 3rd position
         currentMultiplier_buy = local_aggression_factor;
        }
      else // This is for adding the 3rd, 4th, etc.
        {
         newLot = initialLot_buy * currentMultiplier_buy;
         // Dampen the multiplier for the next one
   ...
   if(EnableExponentialScaling)
     {
      ...
      if(positionsCount_sell == 1) // This is for adding the 2nd position
        {
