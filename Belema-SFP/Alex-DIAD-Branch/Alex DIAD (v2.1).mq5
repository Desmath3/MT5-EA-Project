//+------------------------------------------------------------------+
//| Belema SFP EA (Enhanced) - v2.1 with Virtual Trailing Stop       |
//| Swing Failure Pattern with Scaling In                            |
//| Built with features from Power Pivot EA v3.6 by Belema           |
//| Version 2.1 includes a dynamic, virtual ATR-based trailing stop. |
//+------------------------------------------------------------------+
#property copyright "Belema"
#property link      "https://www.mql5.com"
#property version   "2.1"
#property description "Combines SFP logic with scaling in to double account."
#property description "Removes prior trade management; focuses on initial entry and scaling."
#property description "v2.1: Added virtual ATR-based Trailing Stop for profit protection."
...
void CheckForBoS()
  {
   ...
   if(bearish_bos)
     {
      Print("Bearish BoS detected. Validating all resistance levels.");
      for(int i = 0; i < ArraySize(levels); i++)
        {
   ...
   if(bullish_bos)
     {
      Print("Bullish BoS detected. Validating all support levels.");
      for(int i = 0; i < ArraySize(levels); i++)
        {
         if(!levels[i].isResistance) levels[i].validated = true;
        }
...
// Prevent same direction entry if already open
   if(isBuy && HasOpenBuyPositions())
     {
      Print("Trade Blocked: Buy positions already open.");
      return false;
     }
   if(!isBuy && HasOpenSellPositions())
     {
      Print("Trade Blocked: Sell positions already open.");
      return false;
     }
   ...
   if(!EnableHedge)
     {
      if(isBuy && HasOpenSellPositions())
        {
         Print("Trade Blocked (Hedging OFF): Sell positions open.");
         return false;
        }
      if(!isBuy && HasOpenBuyPositions())
        {
         Print("Trade Blocked (Hedging OFF): Buy positions open.");
         return false;
        }
     }
...
void UpdateBuyBreakeven()
  {
   ...
   double bePrice = sumLotEntry / totalLot;
   double offset = BreakevenOffsetPips * _Point;
   double newSL = bePrice + offset;
...
void ManageTrailingStop(bool isBuy)
{
    ...
    if(isBuy && !HasOpenBuyPositions()) return;
    if(!isBuy && !HasOpenSellPositions()) return;
...
void OnTick()
  {
   ...
   // Check for virtual stop loss hits first
   CheckVirtualStops();
       
   // Then, manage scaling and update trailing stop levels
   ManageBuyScaling();
   ManageSellScaling();
   ManageTrailingStop(true);
   ManageTrailingStop(false);
   
   // Check for new pending entries if applicable
   if(ExecutionType != AGGRESSIVE && !inSleepMode)
     {
      CheckActiveSignals();
     }
     
   // Clean up expired level objects from chart
   for(int i = ArraySize(levels) - 1; i >= 0; i--)
     {
      if(levels[i].expired)
        {
