//+------------------------------------------------------------------+
//| Belema SFP EA (Enhanced) - v2.5 with RR-Triggered Profit Keeping |
//| Swing Failure Pattern with Scaling In                            |
//| v2.5: Removed Runner. Added RR Trigger for Peak Drawdown.        |
//| 1. Progressive Profit-Lock Breakeven (Corrected Calculation)     |
//| 2. Peak Profit Drawdown Exit (RR-Triggered)                      |
//| 3. Time-Based Stop for Stale Trades                              |
//+------------------------------------------------------------------+
...
#property description "v2.5: Removed Runner logic, added RR trigger for Peak Drawdown."
...
input group "Profit Keeper: Progressive BE"
input bool   EnableProgressiveBE = true;      // Enable Progressive Profit-Lock BE
...
input double StaleTradeMinProfitR = 1.0;      // Min profit (in R-units) to avoid stale exit
...
void AddBuyPosition()
  {
   ...
   if(Trade.Buy(newLot, _Symbol, 0, 0, 0, comment))
     {
...
void AddSellPosition()
  {
   ...
   if(Trade.Sell(newLot, _Symbol, 0, 0, 0, comment))
     {
...
//+------------------------------------------------------------------+
void ManageProgressiveBE(bool isBuy)
  {
   if(!EnableProgressiveBE) return;
   int posCount = isBuy ? positionsCount_buy : positionsCount_sell;
   ...
   double newSL = isBuy ? bePrice + profitInPoints : bePrice - profitInPoints;
   newSL = NormalizeDouble(newSL, _Digits);
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      if(PositionInfo.SelectByIndex(i) && PositionInfo.Symbol() == _Symbol && PositionInfo.Magic() == EA_MagicNumber && PositionInfo.PositionType() == type)
        {
         if(isBuy && newSL > PositionInfo.StopLoss())
            Trade.PositionModify(PositionInfo.Ticket(), newSL, 0);
         if(!isBuy && (newSL < PositionInfo.StopLoss() || PositionInfo.StopLoss() == 0))
            Trade.PositionModify(PositionInfo.Ticket(), newSL, 0);
        }
     }
  }
...
void ManageStaleTrades(bool isBuy)
  {
   if(!EnableStaleTradeExit) return;
   datetime initialTime = isBuy ? initialTradeTime_buy : initialTradeTime_sell;
   if(initialTime == 0) return;
   int barsOpen = iBarShift(_Symbol, _Period, initialTime, false) - 1;
   if(barsOpen > StaleTradeBars)
     {
      double currentProfit = isBuy ? CalculateBuyProfit() : CalculateSellProfit();
      double initialRisk = isBuy ? initialRiskAmount_buy : initialRiskAmount_sell;
      if(initialRisk > 0 && currentProfit < (initialRisk * StaleTradeMinProfitR))
        {
         Print("STALE TRADE [", (isBuy ? "BUY" : "SELL"), "] EXIT: Trade open for ", barsOpen, " bars without reaching ", StaleTradeMinProfitR, "R profit. Closing positions.");
         if(isBuy) CloseBuyPositions();
         else CloseSellPositions();
        }
     }
      ...
      if(HasOpenBuyPositions())
        {
         ...
         ManageStaleTrades(true);
         ManageProgressiveBE(true);
        }
      if(HasOpenSellPositions())
        {
         ManageTrailingStop(false);
         ManagePeakProfitDrawdown(false);
         ManageStaleTrades(false);
         ManageProgressiveBE(false);
        }
     }
   else
     {
