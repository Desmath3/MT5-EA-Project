...
#property description "Trades Swing Failure Patterns with Multi-Timeframe levels, Power Candles, and FVG. Version 1.01 adds a hedge control parameter."
...
// New Input Parameters for FVG Logic
input bool UseFVGConfirmation = true; // Enable FVG confirmation for trades
input int FindFVGLimit = 10;          // Max candles to find FVG after reclaim power candle
...
void AddLevel(double price, ENUM_TIMEFRAMES tf, datetime time, bool isResistance, datetime end_plot_time) {
    ...
    levels[n].barsBeyondLevel = 0;      // Initialize bars beyond level
    levels[n].expired = false;          // Initialize expired flag
...
// Set limit order with enhanced logging and hedge check
void SetLimitOrder(ENUM_ORDER_TYPE type, double price, double level, bool isResistance) {
    // --- HEDGE CHECK ---
    if (!EnableHedge) {
        if (type == ORDER_TYPE_BUY_LIMIT && HasOpenSellPositions()) {
            Print("Hedging disabled. Cannot place Buy Limit while a Sell position is open.");
            return;
        }
        if (type == ORDER_TYPE_SELL_LIMIT && HasOpenBuyPositions()) {
            Print("Hedging disabled. Cannot place Sell Limit while a Buy position is open.");
            return;
        }
    }
    // --- END HEDGE CHECK ---
    ...
    string comment = "SFP " + tradeType + " at " + DoubleToString(price, _Digits) + " for " + levelType + " level " + DoubleToString(level, _Digits);
    Trade.SetExpertMagicNumber(EA_MagicNumber);
    if (type == ORDER_TYPE_BUY_LIMIT) {
        if (!Trade.BuyLimit(lotSize, price, _Symbol, sl, tp, ORDER_TIME_GTC, 0, comment)) {
            Print("Buy Limit order failed: ", GetLastError());
            return;
        }
    } else if (type == ORDER_TYPE_SELL_LIMIT) {
        if (!Trade.SellLimit(lotSize, price, _Symbol, sl, tp, ORDER_TIME_GTC, 0, comment)) {
    ...
    if (ticket > 0) {
        ...
        // Enhanced logging
        Print("Trade Entered: ", tradeType, " at ", DoubleToString(price, _Digits), 
              " for ", levelType, " level ", DoubleToString(level, _Digits), 
              ". SL: ", DoubleToString(sl, _Digits), ", TP: ", DoubleToString(tp, _Digits), 
              ". Reason: SFP conditions met (Breakout, Reclaim, FVG confirmed).");
    }
}
...
// Process resistance level with expiry check
void ProcessResistanceLevel(int level_idx) {
    ...
    // Check for level expiry
    if (levels[level_idx].expired) return;
    if (current_close > levels[level_idx].price) {
        levels[level_idx].barsBeyondLevel++;
        if (levels[level_idx].barsBeyondLevel > 50) {
            levels[level_idx].expired = true;
            Print("Resistance level at ", DoubleToString(levels[level_idx].price, _Digits), 
                  " expired after 50 bars beyond.");
            ArrayRemove(levels, level_idx, 1);
            return;
        }
    } else {
    ...
    // New trade placement logic
    if (levels[level_idx].reclaimPowerCandleConfirmed && !levels[level_idx].tradePlaced) {
        if (UseFVGConfirmation) {
            int reclaim_shift = iBarShift(_Symbol, _Period, levels[level_idx].reclaimPowerCandleTime);
            int bars_since_reclaim = reclaim_shift - current_bar_shift;
            if (bars_since_reclaim > 0 && bars_since_reclaim <= FindFVGLimit) {
                if (HasFVG(current_bar_shift, false)) { // Bearish FVG
                    int middle_shift = current_bar_shift + 1;
                    if (middle_shift < iBars(_Symbol, _Period)) {
                        double middle_high = iHigh(_Symbol, _Period, middle_shift);
                        double middle_low = iLow(_Symbol, _Period, middle_shift);
                        double entry_price = (middle_high + middle_low) / 2.0;
                        SetLimitOrder(ORDER_TYPE_SELL_LIMIT, entry_price, levels[level_idx].price, levels[level_idx].isResistance);
                        levels[level_idx].tradePlaced = true;
                        TradesToday++;
                        Comment("SFP: SELL LIMIT placed for ", DoubleToString(levels[level_idx].price, _Digits), 
                                " at ", DoubleToString(entry_price, _Digits), " based on FVG at candle ", current_bar_shift);
                    }
                }
            }
        } else {
            int power_shift = iBarShift(_Symbol, _Period, levels[level_idx].reclaimPowerCandleTime);
            double power_high = iHigh(_Symbol, _Period, power_shift);
            double power_low = iLow(_Symbol, _Period, power_shift);
            double entry_price = power_low + MathAbs(power_high - power_low) / 2.0; // 50% of abs(high - low)
            SetLimitOrder(ORDER_TYPE_SELL_LIMIT, entry_price, levels[level_idx].price, levels[level_idx].isResistance);
            levels[level_idx].tradePlaced = true;
            TradesToday++;
            Comment("SFP: SELL LIMIT placed for ", DoubleToString(levels[level_idx].price, _Digits), 
                    " at ", DoubleToString(entry_price, _Digits), " based on reclaim power candle");
        }
    }
}

// Process support level with expiry check
void ProcessSupportLevel(int level_idx) {
    ...
    if (current_close < levels[level_idx].price) {
        ...
        if (levels[level_idx].barsBeyondLevel > 50) {
            levels[level_idx].expired = true;
            Print("Support level at ", DoubleToString(levels[level_idx].price, _Digits), 
                  " expired after 50 bars beyond.");
            ArrayRemove(levels, level_idx, 1);
            return;
        }
    ...
    if (levels[level_idx].reclaimPowerCandleConfirmed && !levels[level_idx].tradePlaced) {
        if (UseFVGConfirmation) {
            ...
            int bars_since_reclaim = reclaim_shift - current_bar_shift;
            if (bars_since_reclaim > 0 && bars_since_reclaim <= FindFVGLimit) {
                if (HasFVG(current_bar_shift, true)) { // Bullish FVG
                    int middle_shift = current_bar_shift + 1;
                    if (middle_shift < iBars(_Symbol, _Period)) {
                        double middle_high = iHigh(_Symbol, _Period, middle_shift);
                        double middle_low = iLow(_Symbol, _Period, middle_shift);
                        double entry_price = (middle_high + middle_low) / 2.0;
                        SetLimitOrder(ORDER_TYPE_BUY_LIMIT, entry_price, levels[level_idx].price, levels[level_idx].isResistance);
                        levels[level_idx].tradePlaced = true;
                        TradesToday++;
                        Comment("SFP: BUY LIMIT placed for ", DoubleToString(levels[level_idx].price, _Digits), 
                                " at ", DoubleToString(entry_price, _Digits), " based on FVG at candle ", current_bar_shift);
                    }
                }
            }
...
void OnTick() {
    ...
    if (currentBarTime != LastBarTime) {
        ...
        if (TradesToday >= MaxTradesPerDay || !IsWithinSession()) {
            // Still manage existing positions and orders even if no new trades are allowed
        } else {
             UpdateLevels();
             for (int i = ArraySize(levels) - 1; i >= 0; i--) { // Iterate backwards for safe removal
    ...
    // These should run on every tick, not just on a new bar
    CheckBreakeven();
    CheckReversalExit();
    EnforceHedgeRule(); // Enforce the no-hedge rule on every tick
}
