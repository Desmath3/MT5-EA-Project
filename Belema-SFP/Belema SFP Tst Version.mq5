...
// Set limit order with enhanced logging
...
void ProcessResistanceLevel(int level_idx) {
    ...
    if (levels[level_idx].reclaimPowerCandleConfirmed && !levels[level_idx].fvgFoundOnReclaim && !levels[level_idx].tradePlaced) {
        if (HasFVG(levels[level_idx].reclaimPowerCandleShift, false)) {
            ...
            TradesToday++;
            Comment("SFP: SELL LIMIT placed for ", DoubleToString(levels[level_idx].price, _Digits), " at ", DoubleToString(entry_price, _Digits));
        }
    }
}
...
void ProcessSupportLevel(int level_idx) {
    ...
    if (levels[level_idx].reclaimPowerCandleConfirmed && !levels[level_idx].fvgFoundOnReclaim && !levels[level_idx].tradePlaced) {
        if (HasFVG(levels[level_idx].reclaimPowerCandleShift, true)) {
            ...
            levels[level_idx].tradePlaced = true;
            TradesToday++;
            Comment("SFP: BUY LIMIT placed for ", DoubleToString(levels[level_idx].price, _Digits), " at ", DoubleToString(entry_price, _Digits));
        }
    }
}
...
int OnInit() {
   ...
   datetime ExpirationDate = StringToTime("2025.08.08 23:59:59");
...
void OnTick() {
    ...
    CheckReversalExit();
       ...
       // Expiration date: August 8th, 2025 23:59:59 GMT
   datetime ExpirationDate = StringToTime("2025.08.08 23:59:59");
   if(TimeCurrent() > ExpirationDate)
   {
      Print("EA expired. Trading disabled, please send an email to @ighodaloxauusd@gmail.com or contact @__ighodalo on X(twitter");
