...
input TRADE_MODE EaTradeMode = Normal;
input int FlipFallbackCandles = 6; // Number of H4 candles for fallback in Flip mode
// Optimization Criterion
enum OPTIM_CRIT { OptNone, MinBalanceDD, MaxProfitVsDD };
input OPTIM_CRIT OptimizationCriterion = OptNone;
...
private:
   ...
   bool OpenOrder(ENUM_ORDER_TYPE orderType, double volume, long magic)
   {
      ...
      if(IsNoTradeZone())
      {
         Print("No-trade zone (NY close). Cannot open order.");
         return false;
      }
      ...
      {
         Print("Premium/Discount filter blocked ", EnumToString(orderType), " order.");
         return false;
      }
   ...
   void UpdateGroupState(TradeGroup &group, ENUM_POSITION_TYPE posType)
   {
      ...
      if(MathAbs(newAvgEntry - group.entryPrice) > _Point || group.virtualTP == 0.0)
      {
         ...
         Print(EnumToString(posType), " group updated: New average entry = ", DoubleToString(newAvgEntry, _Digits), ", New TP = ", DoubleToString(group.virtualTP, _Digits), " (extra pips: ", DoubleToString(extra_pips, 2), ")");
         m_state_changed = true;
      }
   }
   ...
   void CalculateThresholds()
   {
      ...
      {
         Print("Filter: Failed to copy historical data.");
         return;
      }
      ...
      if(rangeSize == 0)
      {
         Print("Filter: Zero range size. Skipping.");
         return;
      }
   ...
   bool IsUS_DST(datetime gmt_time)
   {
      MqlDateTime mt;
      TimeToStruct(gmt_time, mt);
      int year = mt.year;
      // March second Sunday
      datetime march1 = StringToTime(StringFormat("%d.03.01 00:00:00", year));
      MqlDateTime m1;
      TimeToStruct(march1, m1);
      int dow_march1 = m1.day_of_week;
      int second_sunday_march = 8 + (7 - (dow_march1 - 0) % 7) % 7 + 7;
      datetime dst_start = StringToTime(StringFormat("%d.03.%02d 02:00:00", year, second_sunday_march));
      // November first Sunday
      datetime nov1 = StringToTime(StringFormat("%d.11.01 00:00:00", year));
      TimeToStruct(nov1, m1);
      int dow_nov1 = m1.day_of_week;
      int first_sunday_nov = 1 + (7 - (dow_nov1 - 0) % 7) % 7;
      datetime dst_end = StringToTime(StringFormat("%d.11.%02d 02:00:00", year, first_sunday_nov));
      if (gmt_time >= dst_start && gmt_time < dst_end) return true;
      return false;
   }
   bool IsNoTradeZone()
   {
      datetime gmt = TimeGMT();
      bool dst = IsUS_DST(gmt);
      int offset = dst ? -4 : -5;
      datetime ny_time = gmt + offset * 3600;
      MqlDateTime ny;
      TimeToStruct(ny_time, ny);
      int h = ny.hour;
      int m = ny.min;
      return (h == 16 && m >= 30) || (h == 17 && m < 30);
   }
public:
   CBBMartingaleEA()
   {
...
double OnTester()
{
   if(OptimizationCriterion == OptNone) return 0.0;
   double balanceDDPct = TesterStatistics(STAT_BALANCE_DDREL_PERCENT);
   if(OptimizationCriterion == MinBalanceDD) return -balanceDDPct;
   double profit = TesterStatistics(STAT_PROFIT);
   double balanceDDAbs = TesterStatistics(STAT_BALANCE_DD);
   if(balanceDDAbs == 0) return 0.0;
   return profit / balanceDDAbs;
}
//+------------------------------------------------------------------+