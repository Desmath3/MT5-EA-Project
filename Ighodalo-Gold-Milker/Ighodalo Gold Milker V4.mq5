...
private:
   ...
   bool OpenOrder(ENUM_ORDER_TYPE orderType, double volume, long magic)
   {
      ...
      if(IsNoTradeZone())
      {
         Print("No-trade zone. Cannot open order.");
         return false;
      }
   ...
   void UpdateGroupState(TradeGroup &group, ENUM_POSITION_TYPE posType)
   {
      ...
      if(MathAbs(newAvgEntry - group.entryPrice) > _Point || group.virtualTP == 0.0)
      {
         ...
         Print(EnumToString(posType), " group updated: New average entry = ", DoubleToString(newAvgEntry, _Digits), ", New TP = ", DoubleToString(group.virtualTP, _Digits), " (extra pips: ", extra_pips, ")");
         m_state_changed = true;
      }
   }
   ...
   void ManageBuySequenceBreakeven()
   {
      ...
      if(profit >= m_buy_sequence.max_risk_amount * m_sequenceBE_StartR)
      {
         ...
         double new_sl = avg_entry + m_breakevenOffsetPips * PIP_SIZE;
         for(int j = 0; j < m_maxBaskets; j++)
         {
   ...
   void ManageBuyPeakDrawdown()
   {
      ...
      if(profit >= m_buy_sequence.max_risk_amount * m_peakDrawdownStartR)
      {
         ...
         {
            Print("Peak drawdown exit for buys.");
            CloseAllBuys();
         }
      }
   }
   void ManageSellPeakDrawdown()
   {
      ...
      if(profit >= m_sell_sequence.max_risk_amount * m_peakDrawdownStartR)
      {
         ...
         {
            Print("Peak drawdown exit for sells.");
            CloseAllSells();
         }
      }
   }
   ...
   void OnTick()
   {
      ...
      if(is_new_bar)
      {
         ...
         if(m_enableSafeMode)
         {
            double atr = GetCustomATR();
            if(atr <= 0) return;
            double sl_pips = atr * m_atrSlMultiplier / PIP_SIZE;
         ...
         if(m_trade_mode == -1 && !AnySellBasketActive() && !HasAnySellPos() && (m_tradeDirection != TradeBuyOnly) && (m_enableHedge || !HasAnyBuyPos()))
         {
            if(m_enableSafeMode)
            {
               ...
               double proposed_risk = dynLot * (GetCustomATR() * m_atrSlMultiplier) * (SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_VALUE) / SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_SIZE));
               double balance = AccountInfoDouble(ACCOUNT_BALANCE);
               if(current_risk + proposed_risk > balance * m_maxTotalRiskPct / 100.0)
               {
                  Print("Cannot open new sell: Exceeds max total risk.");
                  return;
               }
            }
         ...
         if(m_trade_mode == 1 && !AnyBuyBasketActive() && !HasAnyBuyPos() && (m_tradeDirection != TradeSellOnly) && (m_enableHedge || !HasAnySellPos()))
         {
            if(m_enableSafeMode)
            {
               ...
               if(current_risk + proposed_risk > balance * m_maxTotalRiskPct / 100.0)
               {
                  Print("Cannot open new buy: Exceeds max total risk.");
                  return;
               }
            }
            ...
            if(diff >= threshold)
            {
               ...
               if(i < m_maxBaskets - 1 && m_buy_baskets[i].step >= m_basketThreshold && !m_buy_baskets[next].active)
               {
                  ...
                  if(m_enableSafeMode)
                  {
                     ...
                     {
                        Print("Cannot open new buy basket: Exceeds max total risk.");
                        continue;
                     }
                  }
               ...
               {
                  double current_risk = GetBuyTotalPotentialLoss();
                  double proposed_risk = vol * (GetCustomATR() * m_atrSlMultiplier) * (SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_VALUE) / SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_SIZE));
                  double balance = AccountInfoDouble(ACCOUNT_BALANCE);
                  if(current_risk + proposed_risk > m_buy_sequence.max_risk_amount)
                  {
                     Print("Cannot add martingale buy: Exceeds max total risk.");
                     continue;
                  }
               }
            ...
            if(diff >= threshold)
            {
               ...
               if(i < m_maxBaskets - 1 && m_sell_baskets[i].step >= m_basketThreshold && !m_sell_baskets[next].active)
               {
                  ...
                  if(m_enableSafeMode)
                  {
                     ...
                     {
                        Print("Cannot open new sell basket: Exceeds max total risk.");
                        continue;
                     }
                  }
               ...
               if(m_enableSafeMode)
               {
                  ...
                  if(current_risk + proposed_risk > m_sell_sequence.max_risk_amount)
                  {
                     Print("Cannot add martingale sell: Exceeds max total risk.");
                     continue;
                  }
               }
