...
private:
   ...
   // Logging
   string m_logBuffer;
   datetime m_lastLogDumpTime;
   datetime m_lastGroupStateLogTime;
   // Session Management
   bool m_tradeNewYork;
   bool m_tradeLondon;
   bool m_tradeTokyo;
   ...
   {
      m_logBuffer += TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS) + ": " + message + "\n";
   }
   ...
   double GetCustom35MinATR()
   {
      ...
      if(CopyRates(_Symbol, PERIOD_M5, 0, m5_bars_needed, m5_rates) < m5_bars_needed)
        {
         Log("Not enough M5 history to calculate custom 35-min ATR.");
         return -1.0;
        }
      double sum_tr = 0;
      for(int i = 0; i < m_atrPeriod; i++)
        {
         ...
         if(prev_end_idx >= ArraySize(m5_rates))
           {
            Log("Not enough M5 rates for previous close in custom ATR calc.");
            return -1.0;
           }
         double high = 0;
         double low = 1e10;
   ...
   void CloseGroup(TradeGroup &group, ENUM_POSITION_TYPE posType)
   {
      ...
      for(int i = total - 1; i >= 0; i--)
      {
         ...
         if(ptype == posType)
         {
            ...
            {
               Log("Failed to close position #" + IntegerToString(ticket) + ": " + m_trade.ResultRetcodeDescription());
               allClosed = false;
            }
         }
      }
      ...
      else
      {
         ...
         m_state_changed = true;
         Log("Some positions in " + EnumToString(posType) + " group failed to close. Marked pending_close.");
      }
   }
   ...
   bool OpenOrder(ENUM_ORDER_TYPE orderType, double volume, long magic)
   {
      if(!IsWithinSession())
      {
         Log("Trade blocked by session filter.");
         return false;
      }
      if(!IsMarketOpen())
      {
         Log("Market is closed. Cannot open order.");
         return false;
      }
      if(IsNoTradeZone())
      {
         Log("No-trade zone. Cannot open order.");
         return false;
      }
      ...
      {
         Log("Insufficient margin to open order.");
         return false;
      }
      bool allowBuy = false;
      ...
      {
         Log("Premium/Discount filter blocked " + EnumToString(orderType) + " order.");
         return false;
      }
      ...
      {
         Log("Order opened: " + EnumToString(orderType) + " volume: " + DoubleToString(volume, 2) + " price: " + DoubleToString(price, _Digits) + " magic:" + IntegerToString(magic));
      ...
      {
         Log("Failed to open order: " + IntegerToString(m_trade.ResultRetcode()) + ", " + m_trade.ResultRetcodeDescription());
         return false;
      }
   }
   void UpdateGroupState(TradeGroup &group, ENUM_POSITION_TYPE posType)
   {
      ...
      {
         if(TimeCurrent() - m_lastGroupStateLogTime >= 600)
         {
            Log("Detected position change in " + EnumToString(posType) + " group. Recalculating state.");
            m_lastGroupStateLogTime = TimeCurrent();
         }
         ...
         if(TimeCurrent() - m_lastGroupStateLogTime >= 600)
         {
            Log(EnumToString(posType) + " group updated: New average entry = " + DoubleToString(newAvgEntry, _Digits) + ", New TP = " + DoubleToString(group.virtualTP, _Digits) + " (extra pips: " + DoubleToString(extra_pips, 2) + ")");
         }
         m_state_changed = true;
      }
   }
   void ManageBuyBasket(TradeGroup &group)
   {
      ...
      {
         Log("Retrying pending close for buy group magic " + IntegerToString(group.original_magic));
      ...
      if(!HasPosForGroup(group, POSITION_TYPE_BUY))
      {
         if(group.active && (TimeCurrent() - group.last_trade_time > 5))
         {
            long base = group.original_magic;
            ResetTradeGroup(group, base);
            Log("Buy group was active but no positions found after grace period. Resetting.");
         }
         return;
      }
      ...
      bool use_tp = (EaTradeMode == Normal);
      if(EaTradeMode == Flip)
      {
         int bars_passed = Bars(_Symbol, PERIOD_H4, group.open_time, TimeCurrent());
         if(bars_passed >= FlipFallbackCandles) use_tp = true;
      }
      if(use_tp && group.virtualTP != 0.0 && currentPrice >= group.virtualTP)
      {
         if(IsNoTradeZone()) return;
         Log("Virtual TP hit for BUY group at " + DoubleToString(group.virtualTP, _Digits) + ". Closing group.");
         CloseGroup(group, POSITION_TYPE_BUY);
         return;
      }
   ...
   void ManageSellBasket(TradeGroup &group)
   {
      if(group.pending_close && IsMarketOpen())
      {
         Log("Retrying pending close for sell group magic " + IntegerToString(group.original_magic));
      ...
      if(!HasPosForGroup(group, POSITION_TYPE_SELL))
      {
         if(group.active && (TimeCurrent() - group.last_trade_time > 5))
         {
            long base = group.original_magic;
            ResetTradeGroup(group, base);
            Log("Sell group was active but no positions found after grace period. Resetting.");
         }
         return;
      }
      ...
      {
         if(IsNoTradeZone()) return;
         Log("Virtual TP hit for SELL group at " + DoubleToString(group.virtualTP, _Digits) + ". Closing group.");
         CloseGroup(group, POSITION_TYPE_SELL);
         return;
      }
      if(group.virtualSL != 0.0 && currentPrice >= group.virtualSL)
   ...
   void MergeBaskets(TradeGroup &target, TradeGroup &source, ENUM_POSITION_TYPE posType)
   {
      ...
      Log("Merged " + EnumToString(posType) + " basket.");
      m_state_changed = true;
   }
   void CheckBuyMerges()
   {
   ...
   void LoadNewsFromCache()
     {
      Log("--- NEWS [TESTER MODE]: Initializing news system for backtest. ---");
      ArrayFree(m_cached_news);
      int file_handle = FileOpen(m_news_cache_filename, FILE_READ|FILE_CSV|FILE_COMMON, ',');
      if(file_handle == INVALID_HANDLE)
        {
         Log("--- NEWS [TESTER MODE] ERROR: Could not find news file '" + m_news_cache_filename + "'. Please run the EA on a live chart for 1 minute to create it. ---");
         return;
        }
      // Skip the header row
      ...
      Log("--- NEWS [TESTER MODE]: Successfully loaded " + IntegerToString(count) + " news events into memory for this test run. ---");
     }
   void DownloadAndCacheNews()
     {
      Log("--- NEWS [LIVE MODE]: Initializing. Attempting to download and cache news history for tester use... ---");
      int file_handle = FileOpen(m_news_cache_filename, FILE_WRITE|FILE_CSV|FILE_COMMON, ',');
      if(file_handle == INVALID_HANDLE)
        {
         Log("--- NEWS [LIVE MODE] ERROR: Could not create the cache file. Check terminal permissions. ---");
         return;
        }
      Log("--- NEWS [LIVE MODE]: Cache file '" + m_news_cache_filename + "' opened for writing. ---");
      ...
      Log("--- NEWS [LIVE MODE]: Requesting news history from server. This may take a moment... ---");
      MqlCalendarValue values[];
      int total_saved = 0;
      if(CalendarValueHistory(values, from, to))
        {
         Log("--- NEWS [LIVE MODE]: Received " + IntegerToString(ArraySize(values)) + " total events from server. Filtering and saving... ---");
        ...
        {
         Log("--- NEWS [LIVE MODE] ERROR: Call to CalendarValueHistory failed. Terminal may not be connected or news is disabled in settings. ---");
        }
      FileClose(file_handle);
      Log("--- NEWS [LIVE MODE]: Saved " + IntegerToString(total_saved) + " events. News cache file has been created successfully. You can now use the Strategy Tester. ---");
     }
   datetime GetNextHighImpactNewsTime()
     {
      ...
      if(next_event_time > 0)
        {
         Log("--- NEWS: Next high-impact news for " + _Symbol + " found at: " + TimeToString(next_event_time) + ". ---");
        }
      else
        {
         Log("--- NEWS: No upcoming high-impact news found for " + _Symbol + ". ---");
        }
      return next_event_time;
     }
   void ManagePositionsBeforeNews()
     {
      Log("--- NEWS: Starting position management before high-impact news at " + TimeToString(m_nextNewsTime) + ". ---");
      ...
      if(m_pendingSleep && now_gmt >= m_sleepStart)
        {
         ...
         m_inSleepMode = true;
         Log("--- NEWS: Entering sleep mode. ---");
        }
      if(m_inSleepMode && now_gmt >= m_sleepEndTime)
        {
         m_inSleepMode = false;
         Log("--- NEWS: Exiting sleep mode. ---");
        }
   ...
   void CalculateThresholds()
   {
      ...
      {
         Log("Filter: Failed to copy historical data.");
         return;
      }
      ...
      if(rangeSize == 0)
      {
         Log("Filter: Zero range size. Skipping.");
         return;
      }
...
public:
   ...
   {
      if(m_logBuffer != "") Print(m_logBuffer);
      if(m_ind != INVALID_HANDLE)
      {
   ...
   void OnTick()
   {
      ...
      if(TimeCurrent() - m_lastLogDumpTime >= 14400)
      {
         if(m_logBuffer != "") Print(m_logBuffer);
         m_logBuffer = "";
         m_lastLogDumpTime = TimeCurrent();
      }
      ...
      {
         if(m_day == 0) Log("EA initializing state for existing trades...");
         m_day = dt.day;
         m_startEq = AccountInfoDouble(ACCOUNT_BALANCE);
         m_halted = false;
         if(m_day != 0) Log("Daily reset performed.");
         for(int i = 0; i < m_maxBaskets; i++)
         {
            ...
            {
               Log("Buy basket " + IntegerToString(i) + " found. Re-activating for management.");
            ...
            {
               Log("Sell basket " + IntegerToString(i) + " found. Re-activating for management.");
      ...
      if(is_new_bar)
      {
         ...
         {
            Log("Failed to copy indicator buffers");
            return;
         }
         ...
         if(m_close[1] > m_upper[1])
         {
            if(m_trade_mode != 1)
            {
               Log("Bar closed above upper level. Switching to BUYS ONLY mode.");
               m_trade_mode = 1;
               m_state_changed = true;
            }
         }
         else if(m_close[1] < m_lower[1])
         {
            if(m_trade_mode != -1)
            {
               Log("Bar closed below lower level. Switching to SELLS ONLY mode.");
               m_trade_mode = -1;
               m_state_changed = true;
            }
         }
         ...
         // --- SELL ENTRY LOGIC ---
         if(m_trade_mode == -1 && !AnySellBasketActive() && !HasAnySellPos() && (m_tradeDirection != TradeBuyOnly) && (m_enableHedge || !HasAnyBuyPos()) && TimeCurrent() >= m_sellCooldownEndTime)
         {
            if(m_enableSafeMode)
            {
               ...
               {
                  Log("Cannot open new sell: Exceeds max total risk.");
                  return;
               }
            }
            if(OpenOrder(ORDER_TYPE_SELL, dynLot, m_sell_magics[0]))
            {
               ...
               m_state_changed = true;
               Log("SELL signal: Current candle closed below lower level");
               if(m_enableSafeMode)
               {
                  double balance = AccountInfoDouble(ACCOUNT_BALANCE);
         ...
         // --- BUY ENTRY LOGIC ---
         if(m_trade_mode == 1 && !AnyBuyBasketActive() && !HasAnyBuyPos() && (m_tradeDirection != TradeSellOnly) && (m_enableHedge || !HasAnySellPos()) && TimeCurrent() >= m_buyCooldownEndTime)
         {
            if(m_enableSafeMode)
            {
               ...
               if(current_risk + proposed_risk > balance * m_maxTotalRiskPct / 100.0)
               {
                  Log("Cannot open new buy: Exceeds max total risk.");
                  return;
               }
            }
            if(OpenOrder(ORDER_TYPE_BUY, dynLot, m_buy_magics[0]))
            {
               ...
               m_state_changed = true;
               Log("BUY signal: Current candle closed above upper level");
               if(m_enableSafeMode)
               {
                  double balance = AccountInfoDouble(ACCOUNT_BALANCE);
            ...
            if(diff >= threshold)
            {
               ...
               if(i < m_maxBaskets - 1 && m_buy_baskets[i].step >= m_basketThreshold && !m_buy_baskets[next].active)
               {
                  ...
                  if(m_enableSafeMode)
                  {
                     ...
                     {
                        Log("Cannot open new buy basket: Exceeds max total risk.");
                        continue;
                     }
                  }
                  if(OpenOrder(ORDER_TYPE_BUY, dynLot, m_buy_magics[next]))
                  {
                     ...
                     m_state_changed = true;
                     Log("Started new buy basket " + IntegerToString(next));
                     added = true;
                  }
               }
               ...
               if(m_enableSafeMode)
               {
                  ...
                  if(current_risk + proposed_risk > m_buy_sequence.max_risk_amount)
                  {
                     Log("Cannot add martingale buy: Exceeds max total risk.");
                     continue;
                  }
               }
               if(m_buy_baskets[i].step < m_maxSteps && OpenOrder(ORDER_TYPE_BUY, vol, m_buy_magics[i]))
               {
                  ...
                  m_state_changed = true;
                  Log("BUY martingale step " + IntegerToString(m_buy_baskets[i].step) + " triggered for basket " + IntegerToString(i) + " at threshold " + DoubleToString(threshold / GetPipValue(), 0) + " pips");
                  added = true;
               }
               else
               {
                  Log("Max martingale steps reached for buy basket " + IntegerToString(i) + " (step=" + IntegerToString(m_buy_baskets[i].step) + ").");
               }
               if(!added)
               {
                  Log("Failed to add martingale or new basket for buy basket " + IntegerToString(i));
               }
               if(added)
               {
                  SaveIfChanged();
            ...
            if(diff >= threshold)
            {
               ...
               if(i < m_maxBaskets - 1 && m_sell_baskets[i].step >= m_basketThreshold && !m_sell_baskets[next].active)
               {
                  ...
                  if(m_enableSafeMode)
                  {
                     ...
                     {
                        Log("Cannot open new sell basket: Exceeds max total risk.");
                        continue;
                     }
                  }
                  if(OpenOrder(ORDER_TYPE_SELL, dynLot, m_sell_magics[next]))
                  {
                     ...
                     m_state_changed = true;
                     Log("Started new sell basket " + IntegerToString(next));
                     added = true;
                  }
               }
               ...
               if(m_enableSafeMode)
               {
                  ...
                  if(current_risk + proposed_risk > m_sell_sequence.max_risk_amount)
                  {
                     Log("Cannot add martingale sell: Exceeds max total risk.");
                     continue;
                  }
               }
               if(m_sell_baskets[i].step < m_maxSteps && OpenOrder(ORDER_TYPE_SELL, vol, m_sell_magics[i]))
               {
                  ...
                  m_state_changed = true;
                  Log("SELL martingale step " + IntegerToString(m_sell_baskets[i].step) + " triggered for basket " + IntegerToString(i) + " at threshold " + DoubleToString(threshold / GetPipValue(), 0) + " pips");
                  added = true;
               }
               else
               {
                  Log("Max martingale steps reached for sell basket " + IntegerToString(i) + " (step=" + IntegerToString(m_sell_baskets[i].step) + ").");
               }
               if(!added)
               {
                  Log("Failed to add martingale or new basket for sell basket " + IntegerToString(i));
               }
               if(added)
               {
                  SaveIfChanged();
            ...
            if(preferred_type == ORDER_TYPE_BUY && m_trade_mode == 1 && (m_tradeDirection != TradeSellOnly) && (m_enableHedge || !HasAnySellPos()))
            {
               ...
               if(max_i >= 0 && max_i < m_maxBaskets - 1 && !m_buy_baskets[max_i + 1].active)
               {
                  ...
                  if(m_enableSafeMode)
                  {
                     ...
                     if(current_risk + proposed_risk > m_buy_sequence.max_risk_amount)
                     {
                        Log("Cannot open time-based buy basket: Exceeds max total risk.");
                        return;
                     }
                  }
                  int next = max_i + 1;
                  if(OpenOrder(ORDER_TYPE_BUY, dynLot, m_buy_magics[next]))
                  {
                     ...
                     m_state_changed = true;
                     Log("Time-based trigger: Started new buy basket " + IntegerToString(next));
                     SaveIfChanged();
                     return;
                  }
               }
            ...
            else if(preferred_type == ORDER_TYPE_SELL && m_trade_mode == -1 && (m_tradeDirection != TradeBuyOnly) && (m_enableHedge || !HasAnyBuyPos()))
            {
               ...
               if(max_i >= 0 && max_i < m_maxBaskets - 1 && !m_sell_baskets[max_i + 1].active)
               {
                  ...
                  if(m_enableSafeMode)
                  {
                     ...
                     if(current_risk + proposed_risk > m_sell_sequence.max_risk_amount)
                     {
                        Log("Cannot open time-based sell basket: Exceeds max total risk.");
                        return;
                     }
                  }
                  int next = max_i + 1;
                  if(OpenOrder(ORDER_TYPE_SELL, dynLot, m_sell_magics[next]))
                  {
                     ...
                     m_state_changed = true;
                     Log("Time-based trigger: Started new sell basket " + IntegerToString(next));
                     SaveIfChanged();
                     return;
                  }
               }
