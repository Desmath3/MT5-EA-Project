...
input ENUM_APPLIED_PRICE STSource = PRICE_MEDIAN; // SuperTrend Source
input int STDelayPeriod = 0; // Delay Candles for Band Reset
input int STATRPeriod = 10; // SuperTrend ATR Period
input double STATRMultiplier = 5.0; // SuperTrend ATR Multiplier
...
input bool PlotSuperTrendBands = true; // Plot SuperTrend Bands using objects
input color LowerBandColor = clrLime; // Color for SuperTrend Lower Band
input color UpperBandColor = clrMaroon; // Color for SuperTrend Upper Band
input int BarsToPlot = 500; // Number of historical bars to plot SuperTrend Bands
//+------------------------------------------------------------------+
//| CSuperTrendReversalEA Class |
//+------------------------------------------------------------------+
class CSuperTrendReversalEA
  {
private:
   ...
   // --- Handles & Buffers ---
   int m_ADXHandle;
   double m_lower_band[1];
   double m_upper_band[1];
   // --- SuperTrend Calculation Buffers ---
   double m_st_lower_buffer[];
   double m_st_upper_buffer[];
   double m_st_tr_buffer[];
   double m_st_atr_buffer[];
   int m_prev_calculated;
   // --- SuperTrend Static States ---
   int m_delay_lower;
   double m_old_lower;
   int m_delay_upper;
   double m_old_upper;
   ...
   // --- Plotting Members ---
   bool m_PlotSuperTrendBands;
   color m_LowerBandColor;
   color m_UpperBandColor;
   int m_BarsToPlot;
//+------------------------------------------------------------------+
   ...
   CSuperTrendReversalEA()
     {
      ...
      m_STATRMultiplier = STATRMultiplier;
      ...
      m_PlotSuperTrendBands = PlotSuperTrendBands;
      m_LowerBandColor = LowerBandColor;
      m_UpperBandColor = UpperBandColor;
      m_BarsToPlot = BarsToPlot;
      ArrayResize(m_st_lower_buffer, 0);
      ArrayResize(m_st_upper_buffer, 0);
      ArrayResize(m_st_tr_buffer, 0);
      ArrayResize(m_st_atr_buffer, 0);
      ArraySetAsSeries(m_st_lower_buffer, true);
      ArraySetAsSeries(m_st_upper_buffer, true);
      ArraySetAsSeries(m_st_tr_buffer, true);
      ArraySetAsSeries(m_st_atr_buffer, true);
      m_prev_calculated = 0;
      m_delay_lower = 0;
      m_old_lower = 0;
      m_delay_upper = 0;
   ...
   void Initialize()
     {
      ...
      // Initial SuperTrend computation and plot
      UpdateSuperTrendBands();
      if(m_PlotSuperTrendBands) PlotSTBands();
     }
   ...
   void Deinit()
     {
      ...
      ArrayResize(m_powerCandleObjects, 0);
      // Delete ST plot objects
      ObjectsDeleteAll(0, "ST_Lower_", -1, OBJ_TREND);
      ObjectsDeleteAll(0, "ST_Upper_", -1, OBJ_TREND);
     }
//+------------------------------------------------------------------+
   ...
   void UpdateSTState()
     {
      ...
      m_lower_band[0] = m_st_lower_buffer[1];
      m_upper_band[0] = m_st_upper_buffer[1];
      if(lastClose < m_lower_band[0])
        {
         m_stState = ST_WAIT_BUY;
        }
      else if(lastClose > m_upper_band[0])
        {
   ...
   ENUM_ORDER_TYPE GetSTReversalSignal()
     {
      ...
      m_lower_band[0] = m_st_lower_buffer[1];
      m_upper_band[0] = m_st_upper_buffer[1];
      if(m_stState == ST_WAIT_BUY && lastClose > m_lower_band[0])
        {
         Print("Signal Trigger: ST Reclaim BUY. Close (", DoubleToString(lastClose, _Digits), ") reclaimed the Lower Band (", DoubleToString(m_lower_band[0], _Digits), ").");
         signal = ORDER_TYPE_BUY;
         m_stState = ST_NONE;
        }
      else if(m_stState == ST_WAIT_SELL && lastClose < m_upper_band[0])
        {
         Print("Signal Trigger: ST Reclaim SELL. Close (", DoubleToString(lastClose, _Digits), ") reclaimed the Upper Band (", DoubleToString(m_upper_band[0], _Digits), ").");
   ...
   ENUM_ORDER_TYPE GetWickRejectionSignal()
     {
      ...
      if(high1 > m_upper_band[0] && close1 < m_upper_band[0])
        {
         Print("Signal Trigger: ST Wick Rejection SELL. High (", DoubleToString(high1, _Digits), ") pierced Upper Band (", DoubleToString(m_upper_band[0], _Digits), ") but Close (", DoubleToString(close1, _Digits), ") was below it.");
         signal = ORDER_TYPE_SELL;
        }
      else if(low1 < m_lower_band[0] && close1 > m_lower_band[0])
        {
         Print("Signal Trigger: ST Wick Rejection BUY. Low (", DoubleToString(low1, _Digits), ") pierced Lower Band (", DoubleToString(m_lower_band[0], _Digits), ") but Close (", DoubleToString(close1, _Digits), ") was above it.");
         signal = ORDER_TYPE_BUY;
        }
...
//+------------------------------------------------------------------+
//| SUPER TREND BANDS CALCULATION & PLOTTING |
//+------------------------------------------------------------------+
   void UpdateSuperTrendBands()
     {
      int rates_total = Bars(_Symbol, _Period);
      if(rates_total < 2) return;
      int start_pos = m_prev_calculated > 0 ? m_prev_calculated - 1 : 0;
      if(ArraySize(m_st_lower_buffer) < rates_total)
        {
         int old_size = ArraySize(m_st_lower_buffer);
         ArrayResize(m_st_lower_buffer, rates_total);
         ArrayResize(m_st_upper_buffer, rates_total);
         ArrayResize(m_st_tr_buffer, rates_total);
         ArrayResize(m_st_atr_buffer, rates_total);
         ArrayFill(m_st_lower_buffer, old_size, rates_total - old_size, EMPTY_VALUE);
         ArrayFill(m_st_upper_buffer, old_size, rates_total - old_size, EMPTY_VALUE);
         ArrayFill(m_st_tr_buffer, old_size, rates_total - old_size, EMPTY_VALUE);
         ArrayFill(m_st_atr_buffer, old_size, rates_total - old_size, EMPTY_VALUE);
        }
      datetime time[];
      ...
      for(int i = start_pos; i < rates_total; i++)
        {
         int array_idx = rates_total - 1 - i; // Since series, array[0] is latest
         if(i == 0)
           {
            m_st_tr_buffer[array_idx] = high[array_idx] - low[array_idx];
           }
         else
           {
            m_st_tr_buffer[array_idx] = MathMax(high[array_idx] - low[array_idx], MathMax(MathAbs(high[array_idx] - close[array_idx+1]), MathAbs(low[array_idx] - close[array_idx+1])));
           }
        }
      // Calculate ATR as SMA
      for(int i = start_pos; i < rates_total; i++)
        {
         int array_idx = rates_total - 1 - i;
         if(i < m_STATRPeriod - 1)
           {
            m_st_atr_buffer[array_idx] = EMPTY_VALUE;
            continue;
           }
         double sum = 0.0;
         for(int j = 0; j < m_STATRPeriod; j++)
           {
            sum += m_st_tr_buffer[array_idx + j];
           }
         m_st_atr_buffer[array_idx] = sum / m_STATRPeriod;
        }
      // Calculate bands
      for(int i = start_pos; i < rates_total; i++)
        {
         int array_idx = rates_total - 1 - i;
         if(m_st_atr_buffer[array_idx] == EMPTY_VALUE)
           {
            m_st_lower_buffer[array_idx] = EMPTY_VALUE;
            m_st_upper_buffer[array_idx] = EMPTY_VALUE;
            continue;
           }
         double src_i = 0.0;
         switch(m_STSource)
           {
            case PRICE_CLOSE: src_i = close[array_idx]; break;
            case PRICE_OPEN: src_i = open[array_idx]; break;
            case PRICE_HIGH: src_i = high[array_idx]; break;
            case PRICE_LOW: src_i = low[array_idx]; break;
            case PRICE_MEDIAN: src_i = (high[array_idx] + low[array_idx]) / 2.0; break;
            case PRICE_TYPICAL: src_i = (high[array_idx] + low[array_idx] + close[array_idx]) / 3.0; break;
            case PRICE_WEIGHTED: src_i = (high[array_idx] + low[array_idx] + close[array_idx] + close[array_idx]) / 4.0; break;
           }
         double lower = src_i - (m_STATRMultiplier * m_st_atr_buffer[array_idx]);
         double upper = src_i + (m_STATRMultiplier * m_st_atr_buffer[array_idx]);
         if(i == 0)
           {
            m_st_lower_buffer[array_idx] = lower;
            m_st_upper_buffer[array_idx] = upper;
            continue;
           }
         double lower_prev = m_st_lower_buffer[array_idx + 1];
         double upper_prev = m_st_upper_buffer[array_idx + 1];
         double close_prev = close[array_idx + 1];
         // Lower band
         if(close_prev > lower_prev)
           {
            lower = MathMax(lower, lower_prev);
            m_delay_lower = 0;
           }
         else
           {
            if(m_delay_lower == 0)
              {
               m_old_lower = lower_prev;
              }
            m_delay_lower += 1;
            if(m_delay_lower < m_STDelayPeriod)
              {
               lower = m_old_lower;
              }
           }
         m_st_lower_buffer[array_idx] = lower;
         // Upper band
         if(close_prev < upper_prev)
           {
            upper = MathMin(upper, upper_prev);
            m_delay_upper = 0;
           }
         else
           {
            if(m_delay_upper == 0)
              {
               m_old_upper = upper_prev;
              }
            m_delay_upper += 1;
            if(m_delay_upper < m_STDelayPeriod)
              {
               upper = m_old_upper;
              }
           }
   ...
   void PlotSTBands()
     {
      // Delete all existing ST band objects to prevent accumulation
      ObjectsDeleteAll(0, "ST_Lower_", -1, OBJ_TREND);
      ObjectsDeleteAll(0, "ST_Upper_", -1, OBJ_TREND);
      
      int rates_total = Bars(_Symbol, _Period);
      int start_bar = MathMax(0, rates_total - m_BarsToPlot - 1);
      datetime time[];
      ArraySetAsSeries(time, true);
      CopyTime(_Symbol, _Period, 0, rates_total, time);
      for(int i = start_bar; i < rates_total - 1; i++)
        {
         int array_idx = rates_total - 1 - i;
         int array_idx_next = rates_total - 1 - (i + 1);
         // Lower band segment
         if(m_st_lower_buffer[array_idx] != EMPTY_VALUE && m_st_lower_buffer[array_idx_next] != EMPTY_VALUE)
           {
            string name_lower = "ST_Lower_" + IntegerToString(i);
            // Create from older to newer time
            ObjectCreate(0, name_lower, OBJ_TREND, 0, time[array_idx], m_st_lower_buffer[array_idx], time[array_idx_next], m_st_lower_buffer[array_idx_next]);
            ObjectSetInteger(0, name_lower, OBJPROP_COLOR, m_LowerBandColor);
            ObjectSetInteger(0, name_lower, OBJPROP_STYLE, STYLE_SOLID);
            ObjectSetInteger(0, name_lower, OBJPROP_WIDTH, 2);
            ObjectSetInteger(0, name_lower, OBJPROP_RAY, false);
            ObjectSetInteger(0, name_lower, OBJPROP_SELECTABLE, false);
           }
         // Upper band segment
         if(m_st_upper_buffer[array_idx] != EMPTY_VALUE && m_st_upper_buffer[array_idx_next] != EMPTY_VALUE)
           {
            string name_upper = "ST_Upper_" + IntegerToString(i);
            // Create from older to newer time
            ObjectCreate(0, name_upper, OBJ_TREND, 0, time[array_idx], m_st_upper_buffer[array_idx], time[array_idx_next], m_st_upper_buffer[array_idx_next]);
            ObjectSetInteger(0, name_upper, OBJPROP_COLOR, m_UpperBandColor);
            ObjectSetInteger(0, name_upper, OBJPROP_STYLE, STYLE_SOLID);
            ObjectSetInteger(0, name_upper, OBJPROP_WIDTH, 2);
            ObjectSetInteger(0, name_upper, OBJPROP_RAY, false);
            ObjectSetInteger(0, name_upper, OBJPROP_SELECTABLE, false);
           }
        }
      ChartRedraw(0);
     }
      ...
      if(signal == ORDER_TYPE_BUY)
        {
         ...
         double distance = entryPrice - m_lower_band[0];
         if(distance > maxDist)
           {
            Print("BUY Blocked: Distance from Lower ST (", DoubleToString(distance, _Digits), ") is greater than Max Allowed (", DoubleToString(maxDist, _Digits), ").");
            return false;
           }
        }
      else
        {
         ...
         double distance = m_upper_band[0] - entryPrice;
         if(distance > maxDist)
           {
            Print("SELL Blocked: Distance from Upper ST (", DoubleToString(distance, _Digits), ") is greater than Max Allowed (", DoubleToString(maxDist, _Digits), ").");
            return false;
           }
        }
   ...
   void CheckOppositeSTExit()
     {
      ...
      lower[0] = m_st_lower_buffer[0];
      upper[0] = m_st_upper_buffer[0];
      for(int i = PositionsTotal() - 1; i >= 0; i--)
        {
         if(m_position.SelectByIndex(i) && m_position.Symbol() == _Symbol && m_position.Magic() == m_MagicNumber)
           {
            ...
            bool hit = false;
            if(type == POSITION_TYPE_BUY && tick.ask >= upper[0]) hit = true;
            else if(type == POSITION_TYPE_SELL && tick.bid <= lower[0]) hit = true;
            if(hit)
              {
               if(profit < 0)
                 {
         ...
         if(m_ExecutionMode == CONSERVATIVE)
           {
            if(signal == ORDER_TYPE_BUY)
              {
               m_executionTargetLevel = m_lower_band[0];
              }
            else
              {
               m_executionTargetLevel = m_upper_band[0];
              }
           }
...
//+------------------------------------------------------------------+
CSuperTrendReversalEA eaInstance;
int OnInit()
  {
   datetime ExpirationDate = D'2025.11.17 23:59:59';
   if(TimeCurrent() > ExpirationDate)
