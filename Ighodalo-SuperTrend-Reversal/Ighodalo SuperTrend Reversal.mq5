//+------------------------------------------------------------------+
//| Limitless EA ST V4.36.mq5                                        |
//| Copyright 2025, Ighodalo                                         |
//| @--ighodalo on X                                                 |
//+------------------------------------------------------------------+
#property copyright "Ighodalo"
#property link      "https://www.mql5.com"
#property version   "4.36" // Integrated SuperTrend Bands logic directly into EA; removed Bollinger Bands.
#property description "Trades SuperTrend Bands reversals with power candle confirmation and multi-timeframe filters."
...
#property description "Exit logic now includes ATR-based partials, opposite ST partials/exits, and market structure shift exits."
...
// --- SuperTrend Bands & Entry Logic ---
input group              "SuperTrend Bands & Entry Logic"
input ENUM_APPLIED_PRICE STSource = PRICE_MEDIAN;      // ST Source
input int                STDelayPeriod = 0;            // Delay Candles for Band Reset
input int                STAtrPeriod = 10;             // ST ATR Period
input double             STAtrMult = 5.0;              // ST ATR Multiplier
input bool               TradeWickRejections = true;   // TRADE YOUR STRATEGY: false=Reclaim, true=Wick Rejection
input bool               CheckATRDistance = true;      // Check if entry is too far from ST
input double             MaxATRDistanceMultiplier = 1.4; // Max distance from ST = ATR * Multiplier
// --- Execution Settings ---
...
input bool               EnableOppositeSTExit = true;  // Enable exit/partial at opposite ST
input double             OppositeSTPartialPercent = 80.0; // % to close at opposite ST if in profit
input int                TradeExpiryCandles = 20;      // Trade Expiry in Candles
...
input bool               PlotSuperTrend = true;        // Plot SuperTrend Bands on the chart
input color              UpBandColor = clrLime;        // Color for ST Up Band (lower)
input color              DnBandColor = clrMaroon;      // Color for ST Dn Band (upper)
input int                MaxSTBarsToDisplay = 1000;    // Max bars to display for SuperTrend Bands
...
private:
   ...
   bool              m_EnableOppositeSTExit;
   double            m_OppositeSTPartialPercent;
   ...
   bool              m_PlotSuperTrend;
   color             m_UpBandColor;
   color             m_DnBandColor;
   int               m_MaxSTBarsToDisplay;
   // --- News Filter Inputs ---
   ...
   string            m_upBandObjects[];
   string            m_dnBandObjects[];
   ...
   enum STState { ST_NONE, ST_WAIT_BUY, ST_WAIT_SELL } m_stState;
   int               m_tradesToday;
   datetime          m_lastTradeDay;
   datetime          m_lastBarTime;
 
   // --- SuperTrend Handles & Buffers ---
   double            m_upBand[1];        // For current value
   double            m_dnBand[1];        // For current value
   double            m_upBandBuffer[];   // For calculation history
   double            m_dnBandBuffer[];   // For calculation history
   double            m_trBuffer[];       // For calculation history
   double            m_atrBufferST[];    // For calculation history
   // --- SuperTrend State for calculation ---
   int               m_delay_up;
   double            m_old_up;
   int               m_delay_dn;
   double            m_old_dn;
   ...
   // --- Additional for opposite ST close-based check ---
   datetime          m_lastOppositeSTCheck;
   bool              m_last_had_buy_signal;
   bool              m_last_had_sell_signal;
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| SUPER TREND CALCULATION (INTEGRATED)                             |
//+------------------------------------------------------------------+
   void CalculateSuperTrend()
     {
      ...
      int calculated = ArraySize(m_upBandBuffer);
      if(calculated == rates_total) return; // Already up-to-date
 
      // Resize buffers to match chart bars
      ArrayResize(m_trBuffer, rates_total);
      ArrayResize(m_atrBufferST, rates_total);
      ArrayResize(m_upBandBuffer, rates_total);
      ArrayResize(m_dnBandBuffer, rates_total);
 
      // Fetch price data
      double high[], low[], close[], open[];
      if(CopyHigh(_Symbol, _Period, 0, rates_total, high) < rates_total ||
         CopyLow(_Symbol, _Period, 0, rates_total, low) < rates_total ||
         CopyClose(_Symbol, _Period, 0, rates_total, close) < rates_total ||
         CopyOpen(_Symbol, _Period, 0, rates_total, open) < rates_total)
        {
         Print("Failed to copy price data for SuperTrend.");
         return;
        }
      ...
      int start_pos = (calculated > 1) ? calculated -1 : 0;
      ...
      // --- Loop for new bars only ---
      for(int i = start_pos; i < rates_total; i++)
        {
         // 1. Calculate TR
         if(i == 0)
           {
            m_trBuffer[i] = high[i] - low[i];
           }
         else
           {
            m_trBuffer[i] = MathMax(high[i] - low[i], MathMax(MathAbs(high[i] - close[i-1]), MathAbs(low[i] - close[i-1])));
           }
 
         // 2. Calculate ATR (SMA of TR)
         if(i < m_STAtrPeriod - 1)
           {
            m_atrBufferST[i] = EMPTY_VALUE;
           }
         else
           {
            double sum = 0.0;
            for(int j = 0; j < m_STAtrPeriod; j++)
              {
               sum += m_trBuffer[i - j];
              }
            m_atrBufferST[i] = sum / m_STAtrPeriod;
           }
           
         // 3. Calculate Bands
         if(m_atrBufferST[i] == EMPTY_VALUE)
           {
            m_upBandBuffer[i] = EMPTY_VALUE;
            m_dnBandBuffer[i] = EMPTY_VALUE;
            continue;
           }
         double src_i = 0.0;
         switch(m_STSource)
         ...
         double up = src_i - (m_STAtrMult * m_atrBufferST[i]);
         double dn = src_i + (m_STAtrMult * m_atrBufferST[i]);
         ...
         double up1_1 = m_upBandBuffer[i - 1];
         double dn1_1 = m_dnBandBuffer[i - 1];
      ...
      // Copy latest values to single-element arrays for quick access
      if(rates_total > 0)
        {
         m_upBand[0] = m_upBandBuffer[rates_total - 1];
         m_dnBand[0] = m_dnBandBuffer[rates_total - 1];
        }
     }
//+------------------------------------------------------------------+
   ...
   CSTReversalEA()
     {
      ...
      m_STSource = STSource;
      m_STDelayPeriod = STDelayPeriod;
      m_STAtrPeriod = STAtrPeriod;
      m_STAtrMult = STAtrMult;
      ...
      m_EnableOppositeSTExit = EnableOppositeSTExit;
      m_OppositeSTPartialPercent = OppositeSTPartialPercent;
      ...
      m_PlotSuperTrend = PlotSuperTrend;
      m_UpBandColor = UpBandColor;
      m_DnBandColor = DnBandColor;
      m_MaxSTBarsToDisplay = MaxSTBarsToDisplay;
      ...
      ArrayResize(m_upBandObjects, 0);
      ArrayResize(m_dnBandObjects, 0);
      ...
      m_lastOppositeSTCheck = 0;
      m_last_had_buy_signal = false;
      m_last_had_sell_signal = false;
     }
   void Initialize()
     {
      m_trade.SetExpertMagicNumber(m_MagicNumber);
 
      // Initialize ST buffers
      ArrayResize(m_upBandBuffer, 0);
      ArrayResize(m_dnBandBuffer, 0);
      ArrayResize(m_trBuffer, 0);
      ArrayResize(m_atrBufferST, 0);
   ...
   void Deinit()
     {
      ...
      ArrayResize(m_powerCandleObjects, 0);
      for(int i = ArraySize(m_upBandObjects) - 1; i >= 0; i--)
        {
         ObjectDelete(0, m_upBandObjects[i]);
        }
      ArrayResize(m_upBandObjects, 0);
      for(int i = ArraySize(m_dnBandObjects) - 1; i >= 0; i--)
        {
         ObjectDelete(0, m_dnBandObjects[i]);
        }
      ArrayResize(m_dnBandObjects, 0);
     }
//+------------------------------------------------------------------+
   ...
   void UpdateSTState()
     {
      ...
      if(m_upBand[0] == EMPTY_VALUE || m_dnBand[0] == EMPTY_VALUE)
        {
         // Print("SuperTrend bands not ready."); // Optional: can be noisy
         return;
        }
 
      if(lastClose < m_upBand[0])
        {
         m_stState = ST_WAIT_BUY;
        }
      else if(lastClose > m_dnBand[0])
        {
         m_stState = ST_WAIT_SELL;
        }
     }
   ...
   ENUM_ORDER_TYPE GetTradeSignal()
     {
      if(m_TradeWickRejections)
         ...
         return GetSTReversalSignal();
     }
   ENUM_ORDER_TYPE GetSTReversalSignal()
     {
      ENUM_ORDER_TYPE signal = (ENUM_ORDER_TYPE)-1;
      double lastClose = iClose(_Symbol, _Period, 1);
      if(m_upBand[0] == EMPTY_VALUE || m_dnBand[0] == EMPTY_VALUE) return signal;
      if(m_stState == ST_WAIT_BUY && lastClose > m_upBand[0])
        {
         Print("Signal Trigger: ST Reclaim BUY. Close (", DoubleToString(lastClose, _Digits), ") reclaimed the Up Band (", DoubleToString(m_upBand[0], _Digits), ").");
         signal = ORDER_TYPE_BUY;
         m_stState = ST_NONE;
        }
      else if(m_stState == ST_WAIT_SELL && lastClose < m_dnBand[0])
        {
         Print("Signal Trigger: ST Reclaim SELL. Close (", DoubleToString(lastClose, _Digits), ") reclaimed the Dn Band (", DoubleToString(m_dnBand[0], _Digits), ").");
   ...
   ENUM_ORDER_TYPE GetWickRejectionSignal()
     {
      ...
      if(m_dnBand[0] == EMPTY_VALUE || m_upBand[0] == EMPTY_VALUE) return signal;
 
      if(high1 > m_dnBand[0] && close1 < m_dnBand[0])
        {
         Print("Signal Trigger: ST Wick Rejection SELL. High (", DoubleToString(high1, _Digits), ") pierced Dn Band (", DoubleToString(m_dnBand[0], _Digits), ") but Close (", DoubleToString(close1, _Digits), ") was below it.");
         signal = ORDER_TYPE_SELL;
        }
      else if(low1 < m_upBand[0] && close1 > m_upBand[0])
        {
         Print("Signal Trigger: ST Wick Rejection BUY. Low (", DoubleToString(low1, _Digits), ") pierced Up Band (", DoubleToString(m_upBand[0], _Digits), ") but Close (", DoubleToString(close1, _Digits), ") was above it.");
         signal = ORDER_TYPE_BUY;
        }
...
//+------------------------------------------------------------------+
   ...
   void PlotSuperTrendBands()
     {
      if(!m_PlotSuperTrend) return;
      int rates_total = iBars(_Symbol, _Period);
      int bars_in_buffer = ArraySize(m_upBandBuffer);
      if(bars_in_buffer == 0) return;
 
      int start = MathMax(0, bars_in_buffer - m_MaxSTBarsToDisplay);
 
      // Clean up old objects beyond limit
      while(ArraySize(m_upBandObjects) > m_MaxSTBarsToDisplay)
        {
         string old_name = m_upBandObjects[0];
         ObjectDelete(0, old_name);
         ArrayRemove(m_upBandObjects, 0, 1);
        }
      while(ArraySize(m_dnBandObjects) > m_MaxSTBarsToDisplay)
        {
         string old_name = m_dnBandObjects[0];
         ObjectDelete(0, old_name);
         ArrayRemove(m_dnBandObjects, 0, 1);
        }
 
      for(int i = start; i < bars_in_buffer - 1; i++)
        {
         if(m_upBandBuffer[i] == EMPTY_VALUE || m_upBandBuffer[i+1] == EMPTY_VALUE) continue;
         
         // --- CORRECTED TIME CALCULATION ---
         // Buffers are indexed from past to present (0 is oldest bar)
         // iTime() needs a shift from present to past (0 is current bar)
         int shift1 = rates_total - 1 - i;
         int shift2 = rates_total - 1 - (i + 1);
         datetime t1 = iTime(_Symbol, _Period, shift1);
         datetime t2 = iTime(_Symbol, _Period, shift2);
         // --- END OF CORRECTION ---
 
         string up_name = "ST_UpBand_" + IntegerToString(i);
         if(ObjectFind(0, up_name) == -1)
           {
            if(ObjectCreate(0, up_name, OBJ_TREND, 0, t1, m_upBandBuffer[i], t2, m_upBandBuffer[i + 1]))
              {
               ObjectSetInteger(0, up_name, OBJPROP_COLOR, m_UpBandColor);
               ObjectSetInteger(0, up_name, OBJPROP_STYLE, STYLE_SOLID);
               ObjectSetInteger(0, up_name, OBJPROP_WIDTH, 2);
               ObjectSetInteger(0, up_name, OBJPROP_RAY, false);
               ObjectSetInteger(0, up_name, OBJPROP_SELECTABLE, false);
               int size = ArraySize(m_upBandObjects);
               ArrayResize(m_upBandObjects, size + 1);
               m_upBandObjects[size] = up_name;
              }
           }
         else
           {
            ObjectSetInteger(0, up_name, OBJPROP_TIME, 0, t1);
            ObjectSetDouble(0, up_name, OBJPROP_PRICE, 0, m_upBandBuffer[i]);
            ObjectSetInteger(0, up_name, OBJPROP_TIME, 1, t2);
            ObjectSetDouble(0, up_name, OBJPROP_PRICE, 1, m_upBandBuffer[i + 1]);
           }
 
         if(m_dnBandBuffer[i] == EMPTY_VALUE || m_dnBandBuffer[i+1] == EMPTY_VALUE) continue;
         string dn_name = "ST_DnBand_" + IntegerToString(i);
         if(ObjectFind(0, dn_name) == -1)
           {
            if(ObjectCreate(0, dn_name, OBJ_TREND, 0, t1, m_dnBandBuffer[i], t2, m_dnBandBuffer[i + 1]))
              {
               ObjectSetInteger(0, dn_name, OBJPROP_COLOR, m_DnBandColor);
               ObjectSetInteger(0, dn_name, OBJPROP_STYLE, STYLE_SOLID);
               ObjectSetInteger(0, dn_name, OBJPROP_WIDTH, 2);
               ObjectSetInteger(0, dn_name, OBJPROP_RAY, false);
               ObjectSetInteger(0, dn_name, OBJPROP_SELECTABLE, false);
               int size = ArraySize(m_dnBandObjects);
               ArrayResize(m_dnBandObjects, size + 1);
               m_dnBandObjects[size] = dn_name;
              }
           }
         else
           {
            ObjectSetInteger(0, dn_name, OBJPROP_TIME, 0, t1);
            ObjectSetDouble(0, dn_name, OBJPROP_PRICE, 0, m_dnBandBuffer[i]);
            ObjectSetInteger(0, dn_name, OBJPROP_TIME, 1, t2);
            ObjectSetDouble(0, dn_name, OBJPROP_PRICE, 1, m_dnBandBuffer[i + 1]);
           }
        }
     }
   ...
   bool CheckADXTrend(ENUM_ORDER_TYPE signal)
{
    ...
    // We check if the ADX value is BELOW the threshold.
    // This means we only trade in what the ADX considers a weak-trending or ranging market.
    if (adx_main[0] < m_ADXThreshold)
    {
        // ADX is low, indicating a ranging market suitable for reversals.
        // The trade is allowed.
        Print("ADX Filter: PASSED. ADX value (", DoubleToString(adx_main[0], 2), ") is below threshold (", m_ADXThreshold, ").");
        return true;
    }
    else
    {
        // ADX is high, indicating a strong trend.
        // Since this is a reversal strategy, we block the trade.
        Print("ADXFilter: BLOCKED. ADX value (", DoubleToString(adx_main[0], 2), ") is above threshold (", m_ADXThreshold, "), indicating a strong trend unsuitable for reversals.");
        return false;
    }
}
   ...
   bool CheckATRDistanceFromST(ENUM_ORDER_TYPE signal)
     {
      ...
      if(m_dnBand[0] == EMPTY_VALUE || m_upBand[0] == EMPTY_VALUE) return true;
      ...
      if(signal == ORDER_TYPE_BUY)
        {
         ...
         double distance = entryPrice - m_upBand[0];
         if(distance > maxDist)
           {
            Print("BUY Blocked: Distance from Up Band (", DoubleToString(distance, _Digits), ") is greater than Max Allowed (", DoubleToString(maxDist, _Digits), ").");
            return false;
           }
        }
      else
        {
         ...
         double distance = m_dnBand[0] - entryPrice;
         if(distance > maxDist)
           {
            Print("SELL Blocked: Distance from Dn Band (", DoubleToString(distance, _Digits), ") is greater than Max Allowed (", DoubleToString(maxDist, _Digits), ").");
            return false;
           }
        }
...
//+------------------------------------------------------------------+
   ...
   void CheckOppositeSTExit()
     {
      if(!m_EnableOppositeSTExit) return;
      MqlTick tick;
      if(!SymbolInfoTick(_Symbol, tick)) return;
      if(m_dnBand[0] == EMPTY_VALUE || m_upBand[0] == EMPTY_VALUE) return;
      for(int i = PositionsTotal() - 1; i >= 0; i--)
        {
         if(m_position.SelectByIndex(i) && m_position.Symbol() == _Symbol && m_position.Magic() == m_MagicNumber)
           {
            ...
            bool hit = false;
            if(type == POSITION_TYPE_BUY && tick.ask >= m_dnBand[0]) hit = true;
            else if(type == POSITION_TYPE_SELL && tick.bid <= m_upBand[0]) hit = true;
            if(hit)
              {
               if(profit < 0)
                 {
                  Print("Opposite ST Exit: Closing full losing ", (type == POSITION_TYPE_BUY ? "BUY" : "SELL"), " position #", ticket, " at opposite ST.");
               ...
               else
                 {
                  ...
                  if(already_partial) continue;
                  double volume = m_position.Volume();
                  double close_vol = volume * (m_OppositeSTPartialPercent / 100.0);
                  double vol_step = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_STEP);
                  close_vol = vol_step * MathRound(close_vol / vol_step);
                  if(close_vol > 0 && (volume - close_vol) >= SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN))
                    {
                     if(m_trade.PositionClosePartial(ticket, close_vol))
                       {
                        Print("Opposite ST Partial: Closed ", DoubleToString(close_vol, 2), " lots of position #", ticket);
                        int size = ArraySize(m_partialledTickets);
                        ArrayResize(m_partialledTickets, size + 1);
                        m_partialledTickets[size] = ticket;
                       }
                       ...
                       {
                        Print("Failed partial close at ST: ", GetLastError());
                       }
                    }
                  ...
                  if(m_trade.PositionModify(ticket, new_sl, m_position.TakeProfit()))
                    {
                     Print("Set breakeven after partial at opposite ST for position #", ticket, ". New SL: ", DoubleToString(new_sl, _Digits));
                    }
                  else
                    {
      ...
      for(int i = 0; i < ArraySize(m_trackedPositions); i++)
        {
         ...
         if(m_position.SelectByTicket(ticket))
           {
            int bars_passed = iBarShift(_Symbol, _Period, m_position.Time());
            if(bars_passed >= m_TradeExpiryCandles)
              {
               double entry = m_position.PriceOpen();
               ENUM_POSITION_TYPE type = m_position.PositionType();
               MqlTick tick;
               if(SymbolInfoTick(_Symbol, tick))
                 {
                  if(type == POSITION_TYPE_BUY && tick.bid <= entry)
                    {
                     Print("Trade Expiry Exit: Closing BUY #",ticket," as price retraced to entry after expiry.");
                     m_trade.PositionClose(ticket);
                     m_last_had_buy_signal = true;
                    }
                  else if(type == POSITION_TYPE_SELL && tick.ask >= entry)
                    {
                     Print("Trade Expiry Exit: Closing SELL #",ticket," as price retraced to entry after expiry.");
                     m_trade.PositionClose(ticket);
                     m_last_had_sell_signal = true;
                    }
                 }
              }
           }
   ...
   void PlaceTrade(ENUM_ORDER_TYPE orderType)
     {
      ...
      if(!CheckADXTrend(orderType))
        {
         // Print message is handled inside the function
         return;
        }
      // Print(" - ADX Filter: PASSED"); // Redundant, handled inside function
      ...
      if(!CheckATRDistanceFromST(orderType))
        {
         return;
        }
         ...
         if(position_ticket > 0)
           {
            ...
            m_trackedPositions[tsize].open_time = m_position.Time();
...
//+------------------------------------------------------------------+
   void OnNewBar()
     {
      CalculateSuperTrend(); // Calculate ST first
      ...
      if(signal == (ENUM_ORDER_TYPE)-1)
        ...
        {
         if((signal == ORDER_TYPE_BUY && m_last_had_buy_signal) || (signal == ORDER_TYPE_SELL && m_last_had_sell_signal))
           {
            Print("Ignoring repeated signal for " + (signal == ORDER_TYPE_BUY ? "BUY" : "SELL"));
            return;
           }
         if(signal == ORDER_TYPE_BUY) m_last_had_buy_signal = true;
         else m_last_had_sell_signal = true;
        }
         ...
         if(m_ExecutionMode == CONSERVATIVE)
           {
            if(signal == ORDER_TYPE_BUY)
              {
               m_executionTargetLevel = m_upBand[0];
              }
            else
              {
               m_executionTargetLevel = m_dnBand[0];
              }
           }
