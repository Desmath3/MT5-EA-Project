...
input int  PendingOrderExpiryCandles = 10;
input int  LevelExpiryCandles      = 50;
input double MaxDistanceFromLevelAtrMultiplier = 0.1;
input ENUM_ORDER_EXECUTION_TYPE OrderExecutionType = EXECUTION_TYPE_LIMIT;
...
input int    EA_MagicNumber          = 12345;
input double MinTradeDistanceAtrMultiplier = 0.5;
input bool   EnablePartialClose      = true;
input double PartialCloseAtrMultiplier = 1.0;
input double PartialClosePercent     = 50.0;
input bool   EnableBreakeven         = true;
input double BreakevenTriggerReward = 1.0;
input double BreakevenReward         = 0.1;
input bool   EnableReversalExit      = true;
input int    LookbackHighLow         = 5;
input bool   EnableLossReversalExit = true;
input int    LossLookbackHighLow     = 5;
input bool EnablePremiumDiscountFilter = true;
input int RangeLookback = 20;
input int    PowerCandlePeriods       = 14;
input double PowerCandleMultiplier    = 1.5;
input color  BullPowerColor           = clrLimeGreen;
input color  BearPowerColor           = clrRed;
input color  BullFvgColor             = clrDarkGreen;
input color  BearFvgColor             = clrDarkRed;
input int    MaxFvgsToDisplay         = 5000;
input int    MaxPowerCandlesToDisplay = 1000;
input bool UseFVGConfirmation = true;
input int  FindFVGLimit       = 10;
...
// Loads the news events from the cache file for use in the Strategy Tester.
void LoadNewsFromCache()
  {
   ...
   if(header != "time_as_number")
     {
      Print("--- NEWS [TESTER MODE] ERROR: Invalid file format. Expected header 'time_as_number', got '", header, "'. ---");
      FileClose(file_handle);
      return;
     }
   FileReadString(file_handle); // Skip country header
   FileReadString(file_handle); // Skip importance header
   ...
   while(!FileIsEnding(file_handle))
     {
      ...
      int importance = (int)FileReadString(file_handle);
   ...
   for(int i = 0; i < MathMin(5, ArraySize(g_cached_news)); i++)
     {
      Print("--- NEWS [TESTER MODE] DEBUG: Event ", i + 1, ": Time=", TimeToString(g_cached_news[i].time), 
            ", Country=", g_cached_news[i].country, ", Importance=", g_cached_news[i].importance, " ---");
     }
  }
...
// This function is for debugging and prints all news events for the next week to the log.
void Debug_LogUpcomingWeekNews()
  {
   if(!MQLInfoInteger(MQL_TESTER)) return;

   Print("--- NEWS [DEBUG]: Starting one-time log of all upcoming news for the next 7 days ---");
   datetime now = TimeGMT();
   datetime week_from_now = now + (86400 * 7);
   
   Print("--- NEWS [DEBUG]: Current simulated GMT time is: ", TimeToString(now), " ---");
   if(ArraySize(g_cached_news) > 0)
     {
      Print("--- NEWS [DEBUG]: First news event timestamp from cache: ", TimeToString(g_cached_news[0].time), " ---");
     }
   ...
   for(int i = 0; i < ArraySize(g_cached_news); i++)
     {
      if(g_cached_news[i].time > now && g_cached_news[i].time <= week_from_now)
        {
         if(g_cached_news[i].country == relevant_countries[0] || g_cached_news[i].country == relevant_countries[1])
           {
            string importance_str = "Unknown";
            switch(g_cached_news[i].importance)
              {
               case CALENDAR_IMPORTANCE_NONE:     importance_str = "None"; break;
               case CALENDAR_IMPORTANCE_LOW:      importance_str = "Low"; break;
               case CALENDAR_IMPORTANCE_MODERATE: importance_str = "Moderate"; break;
               case CALENDAR_IMPORTANCE_HIGH:     importance_str = "High"; break;
              }
            Print("--- NEWS [DEBUG]: Event at ", TimeToString(g_cached_news[i].time), " | Country: ", g_cached_news[i].country, 
   ...
   if(found_count == 0)
     {
      Print("--- NEWS [DEBUG]: No relevant news events found in the cache for the first week of the test. ---");
     }
   else
     {
      Print("--- NEWS [DEBUG]: Log complete. Found ", found_count, " relevant events for the next 7 days. ---");
     }
  }

// --- END OF NEWS SYSTEM ---

// [Rest of the code remains unchanged; included for completeness]
...
bool IsPowerCandleTF(int shift, bool isBullish, ENUM_TIMEFRAMES timeframe)
  {
   if(shift + PowerCandlePeriods >= iBars(_Symbol, timeframe))
      return false;
   double sum_range = 0;
   for(int i = 0; i < PowerCandlePeriods; i++)
     {
      if(shift + i >= iBars(_Symbol, timeframe))
         return false;
...
bool HasFVG(int shift, bool isBullish)
  {
   if(shift + 2 >= iBars(_Symbol, _Period))
      return false;
...
void PlotPowerCandle(int shift, bool isBullish)
  {
   if(shift >= iBars(_Symbol, _Period))
      return;
...
void UpdateHTFPowerCandleTrend()
  {
   ...
   for(int i = 1; i < 2000; i++)
     {
      if(i >= iBars(_Symbol, HTFPowerCandleTimeframe))
         break;
