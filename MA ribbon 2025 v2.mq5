...
void   UpdateTradeDirection(); // New: update global trend only on reversal
...
//| This function updates the global variable 'tradeDirection' only  |
//| when a clear reversal occurs:                                     |
//| - If currently LONG, switch to SHORT only when the price closes   |
//|   below the lower Bollinger Band.                                |
//| - If currently SHORT, switch to LONG only when the price closes   |
//|   above the upper Bollinger Band.                                |
//| If not set (NONE), initialize based on the current close.        |
//+------------------------------------------------------------------+
void UpdateTradeDirection()
{
   ...
   // If not yet set, initialize the trade direction
   ...
   {
      // Remain LONG until a reversal is confirmed:
      // Only switch to SHORT if price closes below the lower band.
      if(lastClose < lowerBand[0])
         tradeDirection = SHORT;
   }
   else if(tradeDirection == SHORT)
   {
      // Remain SHORT until reversal:
      // Switch to LONG only if price closes above the upper band.
      if(lastClose > upperBand[0])
         tradeDirection = LONG;
   }
}
...
TradeDirection GetDailyBias()
{
   // Use the daily chart (PERIOD_D1) for bias determination.
      ...
      if(PositionSelectByTicket(ticket))
      {
         ...
         if(profitPips >= PartialCloseProfitPips)
         {
            ...
            if(trade.PositionClosePartial(ticket, volumeToClose))
            {
               ...
               if(SetBreakeven)
               {
                  // For breakeven, set the stop loss at the entry price.
                  double openPrice = PositionGetDouble(POSITION_PRICE_OPEN);
