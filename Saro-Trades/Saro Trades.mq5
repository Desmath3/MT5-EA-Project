...
input bool DebugPerformance = false;
...
private:
   ...
   {
      if(!m_positionsDirty && (TimeCurrent() - m_lastPositionUpdate) < 1)
         return;
      ...
      m_cachedBuyCount = m_buyCount;
      m_cachedSellCount = m_sellCount;
      m_cachedBuyProfit = m_buyProfit;
      m_cachedSellProfit = m_sellProfit;
   ...
   bool OpenOrder(ENUM_ORDER_TYPE orderType, double volume, long magic)
   {
      ...
      {
         Log("Trade skipped: Not in allowed premium/discount zone.");
         return false;
      }
...
public:
   ...
   void OnTick()
   {
      ...
      datetime time[1];
      if(CopyTime(_Symbol, PERIOD_CURRENT, 0, 1, time) > 0 && time[0] != m_last_bar_time)
      {
         ...
         // --- SELL ENTRY LOGIC ---
         if(allowSell && !AnySellBasketActive() && !HasAnySellPos() && (m_tradeDirection != TradeBuyOnly) && (m_enableHedge || !HasAnyBuyPos()) && TimeCurrent() >= m_sellCooldownEndTime && (!m_enableSupportResistance || HasRecentSellSweep()))
         {
         ...
         // --- BUY ENTRY LOGIC ---
         if(allowBuy && !AnyBuyBasketActive() && !HasAnyBuyPos() && (m_tradeDirection != TradeSellOnly) && (m_enableHedge || !HasAnySellPos()) && TimeCurrent() >= m_buyCooldownEndTime && (!m_enableSupportResistance || HasRecentBuySweep()))
         {
